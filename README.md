...
纹理、Mesh 与 HDR 环境图准备会预估 CPU 工作集；纹理在同一输入快照上复核尺寸后解码，纹理和 Mesh 的单次估算上限均为 1 GiB。Mesh 的大描述文件保守预约固定额度，避免主线程解析；后台候选在发布前一直占用队列字节额度。估算不等于精确内存峰值。
...
- Finder／系统文件管理器可将 PNG/JPEG、HDR 环境图、glTF/GLB 拖入 Project，复制到落点目录。
  ...
  单批源文件（含 glTF 依赖）合计上限 512 MiB，重复输入只计一次；编辑器主线程发布并更新索引。
