...
Asset Database 另为每次已提交的资产变化分配单调 `AssetRevision`，与仅用于发现磁盘变化的文件签名分离。Engine 持有通用 `TaskScheduler`；已加载 Mesh 在 Project 刷新后由 Worker 执行缓存读取或 CPU 导入，旧 Mesh 在此期间继续可用，app/editor 每帧由 Owner Thread 消费完成结果。只有 revision 仍匹配的候选才会更新导入缓存、创建 Runtime Mesh 并替换 Registry；启动必需资源仍复用同步 `load_mesh()`。
