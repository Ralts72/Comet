...
项目资产移动由 `AssetManager::move_asset()` 编排：目标必须是 `assets/` 内的相对路径且保持扩展名，source 与相邻 `.meta` 成对移动并复用同一 scan；目标冲突、路径越界、metadata 身份不匹配或快照无法提交时回滚文件，AssetHandle 不因重命名或移动改变。
