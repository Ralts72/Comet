...
“项目 → 重命名项目”可修改 `project.json` 的项目名；保存前会检查文件是否已被外部修改，成功后原子写入。
...
App 与 Play 启动时读取同一配置；外部手改文件后需重启宿主，目前不监视项目配置变化。
编辑器 Edit 模式可通过“项目 → 设置 → 输入”面板添加或修改项目默认动作及绑定，保存时校验并原子写回 `project.json`；
外部已修改项目文件时会拒绝覆盖。按键可以录入或填写，鼠标／手柄控制名可填写；同一控制被其他动作使用时只提示，不强制禁止。
