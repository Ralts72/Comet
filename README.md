...
Play 时同一面板切换为运行画面并隐藏编辑工具。Viewport 通过纯值请求提交可见性、目标尺寸和 Camera 来源：Edit 使用不属于 Scene、不会序列化的 editor camera，Play 使用 Runtime Scene 的 primary Camera；SceneRenderer 不依赖 EditorMode。输入焦点仍由 Viewport/editor 交互层管理，等实际相机控制或游戏 Input System 接入时再形成消费契约，不提前进入 Renderer；
