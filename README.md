...
- `engine/`：引擎核心库，包含 `asset/`、`common/`、`config/`、`core/`、`diagnostics/`、`graphics/`、`render/`、`runtime/` 和 `scene/` 模块；`asset/` 当前提供轻量、不透明的 `AssetHandle` 和带类型校验的最小内存
  ...
  `assets/`、`.comet/`、`ProjectSettings/` 的标准路径契约；`.comet/` 再区分可重建 cache 与当前机器的 editor 状态；`graphics/` 按 command、resource、pipeline 和 synchronization 组织 Vulkan 包装，跨组的 Context、Device、Queue、Swapchain 与 RenderPass 保留为顶层编排对象；`graphics/synchronization/GpuCompletionPoint` 表达可跨上传、渲染和延迟销毁复用的非拥有 GPU 完成 token，`graphics/command/UploadManager` 统一 staging、copy/barrier batch 和 completion 生命周期；`render/resource/` 集中放置 Texture/Mesh 的 CPU DTO、程序化 Mesh、Runtime 对象与设备资源创建边界，`render/scene/` 集中放置场景提取、解析和提交渲染流水线，`render/debug/` 提供无 Editor 依赖的 world-space line/AABB DebugDraw list 与按 FrameSlot 执行的独立 line pipeline。
- `editor/`：ImGui 编辑器入口和面板；Hierarchy 以父子树展示 Scene 并支持拖拽调整层级，Inspector 通过组件描述符
  编辑 Transform、MeshRenderer 和 Camera；一次实体属性控件手势或 Global Translation Gizmo 拖拽会作为单个命令进入有界历史，可通过 Edit 菜单或 `Ctrl/Cmd+Z`、`Ctrl/Cmd+Y` 撤销/重做；Project 资产 Inspector 可编辑 Material 和 Texture 导入设置；Edit Viewport 支持 2D/3D 相机控制、左键选择当前渲染对象、空白点击清除选择、选中 Mesh world AABB 高亮、RGB 世界轴移动，并可在 Viewport 聚焦时按 `F` framing；Project 会在资产源文件树变化时自动刷新，File 菜单可新建、打开和保存 `.scene`；`editor/resources/` 保存不进入
  项目资产数据库的编辑器私有字体等资源。
- `app/`：运行时示例程序入口。
...
再关闭日志系统。Viewport 的鼠标坐标统一通过纯布局契约映射到当前实际纹理像素；工具栏、留白、最大边界和 1x 裁切不可见区域不会进入 camera/picking 输入，resize debounce 期间也不会误用尚未发布的请求分辨率。Edit 模式可在画面内以 RMB 环绕、MMB 平移、滚轮缩放 editor camera；2D/3D 会真正切换正交/透视投影，2D 固定观察轴并使用屏幕平移与正交缩放。左键按下先以连续 texture-pixel 坐标命中 Global Translation Gizmo；命中后沿世界轴拖拽并在释放时只记录一个 UUID + descriptor Transform 命令，带 parent 的实体会把世界位移转换回 local translation，未命中才量化为当前纹理 pixel 并进入 CPU ray-local-AABB 场景拾取，空白点击清除 Selection。该路径不依赖 GPU readback，也不把 ImGui 下沉到引擎。Viewport 聚焦且未编辑文本时，`F` 只触发一次 Focus：Editor 组合根解析选中实体的 world transform 与 Runtime Mesh local bounds，通用 geometry 生成 world AABB，现有 camera controller 按 2D/3D 和纹理 aspect framing，不修改 Scene。同一 world bounds 解析也在 Edit Viewport 可见时向通用 DebugDraw list 提交 12 条深度测试线；Play、资产选择或缺失 Runtime Mesh 不产生高亮，ViewPanel/Scene/Material 均不感知该状态。Gizmo 的 RGB 轴与箭头同样写入 DebugDraw，并按 DPI/纹理显示比例保持固定 logical-pixel 尺寸。Renderer 在 wait/acquire 成功后先执行 ImGui overlay prepare，再通过 Engine 提供的延迟 provider 提取当前 Scene 快照；Inspector/Gizmo 修改、world bounds、Mesh model matrix 和 editor camera 因而在同一帧进入 SceneResolver，draw data 仍在场景之后合成。Runtime Mesh 不保留完整 CPU 顶点副本，但会与 GPU buffer 一起保存可供拾取、Focus 和 culling 复用的局部 AABB。Shader
