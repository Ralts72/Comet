...
- `3rdparty/`：第三方依赖目录，部分依赖通过 Git submodule 拉取，Vulkan Memory Allocator 以 vendored 源码形式维护。
...
- Git submodules，用于拉取 `glfw`、`glm`、`googletest`、`imgui`、`spdlog`、`stb_image` 和 `yaml-cpp`；
  `3rdparty/VulkanMemoryAllocator/` 直接随仓库提交。
...
对外只作为运行时配置加载入口，底层渲染资源不直接读取原始配置。Vulkan 内存分配由 `engine/src/graphics/vulkan_allocator.h`
