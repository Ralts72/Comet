...
namespace Comet::Tests {
    namespace {
        ...
        concept SupportsTimelineWaitModes =
            requires(const T& semaphore, uint64_t value, uint64_t timeout) {
                { semaphore.wait(value) } -> std::same_as<void>;
                { semaphore.wait_for(value, timeout) } -> std::same_as<bool>;
            };

        template<typename T>
        concept SupportsSynchronization2Submit =
            requires(T& queue, std::span<const QueueSemaphoreSubmit> waits,
