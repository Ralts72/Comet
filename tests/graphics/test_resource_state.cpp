...
namespace Comet::Tests {
    ...
    TEST(ResourceStateTest, RequiresExplicitShaderStages) {
        ...
        EXPECT_FALSE(resolve_resource_state(
            ResourceUsage::SampledRead,
            Flags<PipelineStage>(PipelineStage::Transfer)));
    ...
    TEST(ResourceStateTest, ResolvesImageLayoutsAndSynchronization) {
        const auto undefined = resolve_image_state(
            ResourceUsage::Undefined,
            color_range());
        const auto transfer = resolve_image_state(
            ResourceUsage::TransferDestination,
            color_range());
        const auto sampled = resolve_image_state(
            ResourceUsage::SampledRead,
            color_range(),
            Flags<PipelineStage>(PipelineStage::FragmentShader));
        const auto storage = resolve_image_state(
            ResourceUsage::StorageReadWrite,
            color_range(),
            Flags<PipelineStage>(PipelineStage::ComputeShader));
        const auto color = resolve_image_state(
            ResourceUsage::ColorAttachmentWrite,
            color_range());
        const auto depth = resolve_image_state(
            ResourceUsage::DepthStencilAttachmentWrite,
            depth_range());
        const auto depth_read = resolve_image_state(
            ResourceUsage::DepthStencilAttachmentRead,
            depth_range());
        const auto present = resolve_image_state(
        ...
        EXPECT_EQ(
            ...
            Flags<Access>(Access::ColorAttachmentRead)
