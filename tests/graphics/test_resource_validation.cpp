...
namespace Comet::Tests {
    namespace {
        ...
        template<typename T>
        concept SupportsImageReferenceTransition =
            requires(T& context, const Image& image, const ImageState& before,
                const ImageState& after) { context.transition_image_state(image, before, after); };

        template<typename T>
        concept SupportsImagePointerTransition =
            requires(T& context, const Image* image, const ImageState& before,
                const ImageState& after) { context.transition_image_state(image, before, after); };

        template<typename T>
        concept SupportsBufferReferenceTransition = requires(T& context, const Buffer& buffer,
            const ResourceState& before,
            const ResourceState& after) { context.transition_buffer_state(buffer, before, after); };

        template<typename T>
        concept SupportsBufferPointerTransition = requires(T& context, const Buffer* buffer,
            const ResourceState& before,
            const ResourceState& after) { context.transition_buffer_state(buffer, before, after); };
        ...
        template<typename T>
        concept SupportsMultipleVertexBufferBindings =
            requires(const T& command_buffer, const std::span<const VertexBufferBinding> bindings) {
        ...
        template<typename T>
        concept SupportsRenderTargetResize = requires(T& target) { target.resize(1280, 720); };

        template<typename T>
        concept SupportsRenderTargetRecreation = requires(T& target) { target.recreate(); };
        ...
        static_assert(!std::is_invocable_v<CpuBufferFactory, std::nullptr_t, Flags<BufferUsage>,
        ...
        static_assert(
            !std::is_invocable_v<ImageWrapper, std::nullptr_t, vk::Image, const ImageInfo&>);

        static_assert(std::is_invocable_v<CpuBufferFactory, Device&, Flags<BufferUsage>, size_t,
            const void*, std::string_view>);
        static_assert(std::is_invocable_v<ImageFactory, Device&, const ImageInfo&, SampleCount,
            std::string_view>);
        static_assert(std::is_invocable_r_v<GpuResourceResult<std::shared_ptr<Image>>,
            ...
            std::string_view>);
        static_assert(std::is_invocable_v<ImageWrapper, Device&, vk::Image, const ImageInfo&>);
        static_assert(std::is_invocable_r_v<std::shared_ptr<ImageView>, ImageViewFactory, Device&,
        ...
        static_assert(std::is_invocable_r_v<GpuResourceResult<std::unique_ptr<RenderTarget>>,
            RecoverableMultiTargetFactory, Device&, RenderPass&, Math::Vec2u, uint32_t>);
        static_assert(!std::is_constructible_v<OwnedImage, Device&, const ImageInfo&, SampleCount,
            std::string_view>);
        ...
        static_assert(!std::is_constructible_v<FrameBuffer, Device&, RenderPass&,
            const std::vector<std::shared_ptr<ImageView>>&, uint32_t, uint32_t>);
        static_assert(
            !std::is_constructible_v<MultiTarget, Device&, RenderPass&, Math::Vec2u, uint32_t>);
