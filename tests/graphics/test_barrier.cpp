...
namespace Comet::Tests {
    namespace {
        ...
        ImageState require_state(const ResourceUsage usage,
            const ImageSubresourceRange& subresources,
            const Flags<PipelineStage> shader_stages = {}) {
            const auto state = resolve_image_state(usage, subresources, shader_stages);
            EXPECT_TRUE(state);
    ...
    TEST(BufferBarrierTest, BuildsTransferToVertexReadDependency) {
        ...
        EXPECT_EQ(barrier->dstStageMask, vk::PipelineStageFlagBits2::eVertexInput);
        EXPECT_EQ(barrier->dstAccessMask, vk::AccessFlagBits2::eVertexAttributeRead);
    ...
    TEST(BufferBarrierTest, RejectsInvalidScopeRangeOrOwnershipTransfer) {
        auto before = require_resource_state(ResourceUsage::TransferDestination);
        auto after = require_resource_state(ResourceUsage::IndexBuffer);

        EXPECT_FALSE(Graphics::build_buffer_memory_barrier({}, before, after, 0, 0));

        after.stages = {};
        EXPECT_FALSE(Graphics::build_buffer_memory_barrier({}, before, after));
    ...
    TEST(ImageBarrierTest, BuildsSynchronization2BarrierFromTypedStates) {
        ...
        EXPECT_EQ(barrier->dstStageMask, vk::PipelineStageFlagBits2::eTransfer);
        EXPECT_EQ(barrier->dstAccessMask, vk::AccessFlagBits2::eTransferWrite);
        EXPECT_EQ(barrier->oldLayout, vk::ImageLayout::eUndefined);
        EXPECT_EQ(barrier->newLayout, vk::ImageLayout::eTransferDstOptimal);
        EXPECT_EQ(barrier->srcQueueFamilyIndex, VK_QUEUE_FAMILY_IGNORED);
        EXPECT_EQ(barrier->dstQueueFamilyIndex, VK_QUEUE_FAMILY_IGNORED);
    ...
    TEST(ImageBarrierTest, MapsShaderAccess) {
        const auto subresources = color_range();
        auto before = require_state(ResourceUsage::TransferDestination, subresources);
        auto after = require_state(ResourceUsage::SampledRead, subresources,
    ...
    TEST(ImageBarrierTest, RejectsIncompleteOrContradictoryStates) {
        ...
        EXPECT_FALSE(Graphics::build_image_memory_barrier({}, before, after));
