...
function(expect_failure EXPECTED)
    execute_process(COMMAND "${COMPILE_TOOL}" ${ARGN}
        RESULT_VARIABLE RESULT OUTPUT_VARIABLE OUTPUT ERROR_VARIABLE ERROR)
    if (RESULT EQUAL 0 OR NOT "${ERROR}" MATCHES "${EXPECTED}")
        message(FATAL_ERROR "Command did not report ${EXPECTED}: ${OUTPUT}\n${ERROR}")
    endif ()
endfunction()
expect_failure("Required:")
expect_failure("Missing option value" --source)
expect_failure("Unknown option" --unknown value)
expect_failure("Duplicate define" --define VALUE=1 --define VALUE=2)
expect_failure("Unsupported target" --target unsupported)
expect_failure("Unsupported stage" --source "${ROOT}/source.vert" --stage unknown --output "${SPV}")
file(WRITE "${ROOT}/valid.vert" "#version 450\nvoid main(){gl_Position=vec4(1);}\n")
set(INVALID_OUTPUT "${ROOT}/invalid\noutput.spv")
expect_failure("Newline in dependency path" --source "${ROOT}/valid.vert"
    --stage vert --output "${INVALID_OUTPUT}" --depfile "${SPV}.d")
file(SHA256 "${SPV}" AFTER_FAILURE)
file(READ "${SPV}.d" AFTER_DEPFILE)
if (EXISTS "${INVALID_OUTPUT}" OR NOT CHANGED STREQUAL AFTER_FAILURE
    OR NOT ORIGINAL_DEPFILE STREQUAL AFTER_DEPFILE)
    message(FATAL_ERROR "Invalid arguments changed previously published outputs")
endif ()
file(MAKE_DIRECTORY "${ROOT}/blocked")
file(WRITE "${ROOT}/blocked/keep" "original")
expect_failure("Failed to atomically replace file" --source "${ROOT}/valid.vert"
    --stage vert --output "${SPV}" --depfile "${ROOT}/blocked")
file(SHA256 "${SPV}" AFTER_IO_FAILURE)
if (NOT CHANGED STREQUAL AFTER_IO_FAILURE)
    message(FATAL_ERROR "Failed depfile write replaced the previous shader artifact")
endif ()
expect_failure("Failed to atomically replace file" --source "${ROOT}/valid.vert"
    --stage vert --output "${ROOT}/blocked")
file(READ "${ROOT}/blocked/keep" KEPT)
file(GLOB LEFTOVERS "${ROOT}/.comet-tmp-*")
if (NOT KEPT STREQUAL "original" OR LEFTOVERS)
    message(FATAL_ERROR "Failed CLI publication changed the destination or left temporary files")
endif ()
