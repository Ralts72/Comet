...
namespace Comet::Tests {
    ...
    TEST(ImportInputSnapshotTest, CapturesSourceAndSortedDependencies) {
        ...
        const std::filesystem::path source =
            project.write("meshes/model.gltf", "source");
        const std::filesystem::path first =
            project.write("buffers/a.bin", "first");
        const std::filesystem::path second =
            project.write("buffers/b.bin", "second");

        const std::array dependencies{second, first, second};
        const ImportInputSnapshot snapshot = capture_import_inputs(
            project.assets(), source, dependencies);
    ...
    TEST(ImportInputSnapshotTest, DetectsContentChangesWithStableFileSize) {
        const TemporaryInputProject project;
        const std::filesystem::path source =
            project.write("mesh.gltf", "source-a");
    ...
    TEST(ImportInputSnapshotTest, RejectsInputsOutsideAssetRoot) {
        const TemporaryInputProject project;
        const std::filesystem::path source =
            project.write("mesh.gltf", "source");
        ...
        EXPECT_THROW(
            static_cast<void>(capture_import_inputs(
                project.assets(), source, dependencies)),
            std::runtime_error);
    }
}
