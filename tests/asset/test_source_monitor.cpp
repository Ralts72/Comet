...
namespace Comet::Tests {
    ...
    TEST(AssetSourceMonitorTest, ReportsFileChangesOnlyOnce) {
        ...
        EXPECT_EQ(
            monitor.poll_now().state,
            AssetSourceMonitor::PollState::Unchanged);

        directory.write("textures/albedo.png", "first");
        EXPECT_EQ(
            monitor.poll_now().state,
            AssetSourceMonitor::PollState::Changed);
        EXPECT_EQ(
            monitor.poll_now().state,
            AssetSourceMonitor::PollState::Unchanged);
    ...
    TEST(AssetSourceMonitorTest, ReportsRecoveryAfterInitialFailureAsChange) {
        ...
        ASSERT_EQ(
            monitor.poll_now().state,
            AssetSourceMonitor::PollState::Failed);
    ...
    TEST(AssetSourceMonitorTest, ThrottledPollDefersFilesystemWork) {
        ...
        AssetSourceMonitor monitor(
            directory.root(), std::chrono::hours(1));
        ASSERT_EQ(
            monitor.poll().state,
            AssetSourceMonitor::PollState::Unchanged);
