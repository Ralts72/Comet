...
namespace Comet::Tests {
    TEST(MeshBoundsTest, CalculatesFiniteLocalAxisAlignedBounds) {
        ...
        EXPECT_TRUE(TestUtils::Vec3Equal(bounds->minimum, Math::Vec3(-2.0f, -1.0f, -4.0f)));
    ...
    TEST(MeshBoundsTest, DoesNotIncludeOriginOrRequireIndices) {
        ...
        EXPECT_TRUE(TestUtils::Vec3Equal(point_bounds->minimum, data.vertices[0].position));
        EXPECT_TRUE(TestUtils::Vec3Equal(point_bounds->maximum, data.vertices[0].position));
    ...
    TEST(MeshBoundsTest, RejectsEmptyOrNonFinitePositions) {
        ...
        for(const float invalid : {std::numeric_limits<float>::quiet_NaN(),
            ...
            for(int axis = 0; axis < 3; ++axis) {
                ...
                data.vertices.insert(data.vertices.begin(), {.position = Math::Vec3(0.0f)});
                EXPECT_FALSE(calculate_mesh_bounds(data));
            }
        }
    }
