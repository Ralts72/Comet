...
namespace Comet::Tests {
    ...
    protected:
        class Factory: public RenderResourceFactory {
            GpuResourceResult<std::shared_ptr<Mesh>> try_create_mesh(const MeshData&) override {
                ADD_FAILURE() << "File import must not allocate GPU resources";
                return GpuResourceResult<std::shared_ptr<Mesh>>::failure(vk::Result::eErrorUnknown);
            }
        ...
        std::filesystem::path root =
            ...
            / ("comet_external_import_" + std::to_string(AssetHandle::generate().value()));
        ...
        std::filesystem::path mesh(const std::string& uri = "data/model.bin") {
            ...
            std::ofstream(source)
                << R"({"asset":{"version":"2.0"},"buffers":[{"byteLength":42,"uri":")" << uri
    ...
    TEST_F(ExternalFileImportTest, CopiesTextureWithoutExternalMetadataOrSceneMutation) {
        ...
        EXPECT_TRUE(MetadataSerializer{}.save(
            {external_handle, AssetType::Texture, make_default_import_settings(AssetType::Texture)},
        ...
        EXPECT_EQ(MetadataSerializer{}.load(metadata_path(source)).value().handle, external_handle);
    ...
    TEST_F(ExternalFileImportTest, GltfCopiesRelativeBufferAndImageAndDeduplicatesInputs) {
        ...
        EXPECT_EQ(
            read(external / "data/model.bin"), read(paths.assets() / "folder/data/model.bin"));
        EXPECT_FALSE(std::filesystem::exists(paths.assets() / "folder/data/model.bin.meta"));
        EXPECT_TRUE(std::filesystem::exists(source));
    }

    TEST_F(ExternalFileImportTest, ImportsGlbToRoot) {
        std::string json =
            read(std::filesystem::path(COMET_SAMPLE_PROJECT_DIRECTORY) / "assets/meshes/cube.gltf");
        while(json.size() % 4)
            json.push_back(' ');
    ...
    TEST_F(ExternalFileImportTest, RejectsRemoteParentAndAbsoluteDependencies) {
        for(const std::string uri : {"https://example.invalid/buffer.bin", "../outside.bin",
                "/absolute.bin", "data/%2e%2e/model.bin"}) {
    ...
    TEST_F(ExternalFileImportTest, RejectsEscapingDestinationAndSymlinkDependency) {
        ...
        std::error_code error;
        std::filesystem::create_directory_symlink(external, paths.assets() / "link", error);
        if(error)
        ...
        std::filesystem::create_symlink(root / "outside.bin", external / "data/model.bin");
    ...
    TEST_F(ExternalFileImportTest, CopyFailureLeavesNoPublishedFilesOrStagingBatch) {
        ...
        std::filesystem::copy_file(external / "data/model.bin", external / "texture.png/model.bin");
