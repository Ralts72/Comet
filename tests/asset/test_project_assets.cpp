...
namespace Comet::Tests {
    TEST(ProjectAssetsTest, DemoAssetsHaveStableIdentityAndValidReferences) {
        ...
        const std::filesystem::path material_source = paths.assets() / "materials/demo.mat";
        ASSERT_TRUE(std::filesystem::exists(material_source));
        const AssetMetadata material = serializer.load(metadata_path(material_source)).value();
        EXPECT_EQ(material.type, AssetType::Material);
        EXPECT_EQ(material.handle, AssetHandle(11364856686536078871ull));

        const MaterialData material_data = MaterialSerializer{}.load(material_source).value();
        ...
        EXPECT_EQ(material_data.texture_properties.at("u_Texture1"), second_texture.handle);
