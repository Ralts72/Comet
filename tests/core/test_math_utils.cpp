...
TEST(MathUtilsTest, WrapsDegreesToSignedCycle) {
    ...
    EXPECT_TRUE(TestUtils::Vec3Equal(
        wrapped, Math::Vec3(0.0f, -90.0f, 1.0f)));
}
...
TEST(MathUtilsTest, PerspectiveUsesDegreeFov) {
    ...
    const Math::Mat4 actual = Math::perspective(
        fov_degrees, aspect, near_z, far_z);
    const Math::Mat4 expected = glm::perspective(
        glm::radians(fov_degrees), aspect, near_z, far_z);
