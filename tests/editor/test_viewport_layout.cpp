...
namespace CometEditor::Tests {
    ...
    TEST(ViewportLayoutTest, FitsSixteenByNineResolutionInsidePhysicalPanel) {
        ...
        EXPECT_EQ(
            layout.render_resolution,
            Comet::Math::Vec2u(2000, 1125));
        expect_vec2(
            layout.image_display_rect.min,
            Comet::Math::Vec2(0.0f, 118.75f));
        expect_vec2(
            layout.image_display_rect.max,
            Comet::Math::Vec2(1000.0f, 681.25f));
    }

    TEST(ViewportLayoutTest, FixedResolutionDoesNotFollowPanelSize) {
        const ViewportResolutionPolicy fixed{
    ...
    TEST(ViewportLayoutTest, ConstrainsFreeResolutionWithoutChangingAspect) {
        ...
        EXPECT_EQ(
            layout.render_resolution,
            Comet::Math::Vec2u(4096, 2048));
        EXPECT_EQ(
            portrait.render_resolution,
            Comet::Math::Vec2u(1024, 4096));
    }
    ...
    TEST(ViewportLayoutTest, ConstrainsPolicyResultRatherThanPanelInput) {
        ...
        EXPECT_EQ(
            aspect.render_resolution,
            Comet::Math::Vec2u(4096, 2304));
        EXPECT_EQ(
            fixed.render_resolution,
            Comet::Math::Vec2u(4096, 2048));
    }
    ...
    TEST(ViewportLayoutTest, OneToOneUsesOnePhysicalPixelPerDisplayPixel) {
        ...
        expect_vec2(
            layout.image_display_rect.min,
            Comet::Math::Vec2(20.0f, 230.0f));
        expect_vec2(
            layout.image_display_rect.max,
            Comet::Math::Vec2(980.0f, 770.0f));
    }
