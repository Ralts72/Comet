...
namespace CometEditor::Tests {
    ...
    TEST(EditorCameraControllerTest, FocusPerspectiveBoundsPreservesViewDirection) {
        ...
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(camera.target, bounds.center(), 0.0001f));
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(
            Comet::Math::normalize(camera.perspective.position - camera.target), previous_direction,
            0.0001f));
    ...
    TEST(EditorCameraControllerTest, FocusOrthographicBoundsFitsAspect) {
        ...
        const Comet::Math::Vec3 previous_offset = camera.perspective.position - camera.target;
    ...
    TEST(EditorCameraControllerTest, FocusRejectsInvalidBoundsOrAspect) {
        ...
        focus_editor_camera(camera,
            {.minimum = Comet::Math::Vec3(-1.0f), .maximum = Comet::Math::Vec3(1.0f)}, 0.0f);
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(
    ...
    TEST(EditorCameraControllerTest, FocusNarrowAspectRequiresMoreDistance) {
        ...
        EXPECT_GT(Comet::Math::length(camera.perspective.position - camera.target), wide_distance);
    }

    TEST(EditorCameraControllerTest, FocusPointAndLargeBoundsStayWithinLimits) {
        EditorCameraState camera;
        focus_editor_camera(camera, Comet::BoundingBox::from_point(Comet::Math::Vec3(0)), 1.0f);
        EXPECT_NEAR(
            Comet::Math::length(camera.perspective.position - camera.target), 0.05f, 0.0001f);
        const Comet::BoundingBox large{
            .minimum = Comet::Math::Vec3(-1.0e20f), .maximum = Comet::Math::Vec3(1.0e20f)};
        focus_editor_camera(camera, large, 1.0f);
        EXPECT_FLOAT_EQ(Comet::Math::length(camera.perspective.position - camera.target), 10000.0f);
        camera.projection = Comet::RenderCamera::Projection::Orthographic;
        focus_editor_camera(camera, large, 1.0f);
    ...
    TEST(EditorCameraControllerTest, OrbitPreservesTargetAndDistance) {
        ...
        apply_editor_camera_input(
            camera, {.orbit_delta = Comet::Math::Vec2(100.0f, -40.0f), .viewport_height = 600.0f});

        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(camera.target, Comet::Math::Vec3(0.0f)));
    ...
    TEST(EditorCameraControllerTest, OrbitDragKeepsHorizontalDirectionAndInvertsVertical) {
        ...
        EXPECT_NEAR(upward.perspective.position.y, -downward.perspective.position.y, 0.0001f);
    }
    ...
    TEST(EditorCameraControllerTest, PanMovesPositionAndTargetTogether) {
        ...
        apply_editor_camera_input(
            camera, {.pan_delta = Comet::Math::Vec2(80.0f, -25.0f), .viewport_height = 600.0f});

        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(
            camera.perspective.position - previous_position, camera.target - previous_target));
        EXPECT_NEAR(Comet::Math::length(camera.perspective.position - camera.target),
    ...
    TEST(EditorCameraControllerTest, InvalidInputAndPanExtentAreIgnored) {
        ...
        apply_editor_camera_input(
            camera, {.pan_delta = Comet::Math::Vec2(10.0f, 5.0f), .viewport_height = 0.0f});
        apply_editor_camera_input(camera, {.zoom_delta = std::numeric_limits<float>::quiet_NaN()});
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(
            camera.perspective.position, previous.perspective.position));
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(camera.target, previous.target));
        ...
        apply_editor_camera_input(
            camera, {.pan_delta = Comet::Math::Vec2(10.0f, 5.0f), .viewport_height = 600.0f});
        EXPECT_TRUE(Comet::Tests::TestUtils::Vec3Equal(
            camera.perspective.position, previous.perspective.position));
    }
    ...
    TEST(EditorCameraControllerTest, OrthographicModePansAndScalesWithoutOrbit) {
        ...
        apply_editor_camera_input(camera, {.orbit_delta = Comet::Math::Vec2(100.0f, 50.0f),
        ...
        EXPECT_FALSE(Comet::Tests::TestUtils::Vec3Equal(camera.target, Comet::Math::Vec3(0.0f)));
