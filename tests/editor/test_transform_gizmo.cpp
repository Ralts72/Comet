...
namespace {
    ...
    TEST_F(TransformGizmoTest, EachScaleAxisPreviewsIndependentlyAndCancelRestoresStart) {
        ...
        for(int axis = 0; axis < 3; ++axis) {
            SCOPED_TRACE(axis);
            const auto handle = gizmo.handles(entity.get_uuid(), camera, layout)[axis];
            ASSERT_TRUE(handle);
            const auto segment = handle->segments.front();
            ...
            ASSERT_TRUE(update({.cancel = true}));
            EXPECT_EQ(transform.scale, initial);
    ...
    TEST_F(TransformGizmoTest, TransformGesturesRoundTripThroughSceneSerialization) {
        ...
        EXPECT_EQ(reopened->get_parent(restored).get_uuid(), parent.get_uuid());
    ...
    TEST_F(TransformGizmoTest, LocalRotationPreservesNonUniformParentAndOwnScale) {
        ...
        expect_matrix(rotation_matrix(),
            before * Math::Mat3(Math::compose_trs({}, {0, 0, 45}, Math::Vec3(1))));
        const auto preview_ring = gizmo.handles(entity.get_uuid(), camera, layout)[2];
        ASSERT_TRUE(preview_ring);
        ASSERT_EQ(preview_ring->segments.size(), ring.segments.size());
        for(std::size_t index = 0; index < ring.segments.size(); ++index) {
            EXPECT_EQ(preview_ring->segments[index].start, ring.segments[index].start);
            EXPECT_EQ(preview_ring->segments[index].end, ring.segments[index].end);
        }
        ASSERT_TRUE(update({.position = ring.segments[20].start, .released = true}));
        expect_matrix(rotation_matrix(),
