...
namespace Comet::Tests {
    ...
    TEST_F(OffscreenResizeTest, ViewUpdatesPreserveTargetWhenHiddenAndRejectChangesAfterShutdown) {
        auto& renderer = engine->get_renderer();
        ASSERT_TRUE(renderer.enable_offscreen_rendering({160, 120}));
        auto& scene = renderer.get_scene_renderer();
        const auto original = scene.get_offscreen_color_view(0);
        ASSERT_TRUE(renderer.set_render_view({.visible = false, .render_size = {200, 140}}));
        EXPECT_EQ(scene.get_offscreen_color_view(0), original);
        ASSERT_TRUE(renderer.set_render_view({.render_size = {0, 0}}));
        EXPECT_EQ(scene.get_offscreen_color_view(0), original);
        ASSERT_TRUE(renderer.set_render_view({.render_size = {200, 140}}));
        EXPECT_EQ(scene.get_render_target().get_size(), Math::Vec2u(200, 140));
        const auto resized = scene.get_offscreen_color_view(0);
        EXPECT_NE(resized, original);

        renderer.prepare_shutdown();
        const auto rejected = renderer.set_render_view({.render_size = {240, 160}});
        ASSERT_FALSE(rejected);
        EXPECT_FALSE(rejected.error().message.empty());
        EXPECT_EQ(scene.get_offscreen_color_view(0), resized);
        EXPECT_EQ(scene.get_render_target().get_size(), Math::Vec2u(200, 140));
    ...
    TEST_F(OffscreenResizeTest, RejectsRepeatedInvalidSizeAndPreservesActualTarget) {
        ...
        ASSERT_TRUE(scene.resize_offscreen_target(invalid_size, now));
        ...
        ASSERT_TRUE(scene.resize_offscreen_target({200, 140}, now));
        EXPECT_EQ(scene.get_render_target().get_size(), Math::Vec2u(200, 140));
