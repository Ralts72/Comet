...
namespace Comet::Tests {
    ...
    TEST(ScenePickingTest, BuildsPerspectiveWorldRayThroughPixelCenter) {
        const ViewProjectMatrix view_project{.view = Math::look_at(Math::Vec3(0.0f, 0.0f, 3.0f),
                                                 Math::Vec3(0.0f), Math::Vec3(0.0f, 1.0f, 0.0f)),
        ...
        EXPECT_TRUE(TestUtils::Vec3Equal(ray->direction, Math::Vec3(0.0f, 0.0f, -1.0f), 0.0001f));
    ...
    TEST(ScenePickingTest, BuildsParallelOrthographicRaysAtPixelPositions) {
        ...
        const auto top_left = make_world_ray(view_project, Math::Vec2u(0, 0), Math::Vec2u(2, 2));
        ...
        EXPECT_TRUE(TestUtils::Vec3Equal(top_left->direction, bottom_right->direction, 0.0001f));
    }
    ...
    TEST(ScenePickingTest, PicksObjectsAtTheirRenderedPixelsInBothProjections) {
        ...
        const auto view = Math::look_at(Math::Vec3(0, 0, 5), Math::Vec3(0), Math::Vec3(0, 1, 0));
        ...
        for(std::size_t i = 0; i < positions.size(); ++i) {
            candidates[i] = {.entity_id = static_cast<EntityId>(i + 1),
                ...
                .local_bounds = {.minimum = Math::Vec3(-0.1f), .maximum = Math::Vec3(0.1f)}};
        }
        for(const auto& projection : {Math::perspective(60.0f, 4.0f / 3.0f, 0.1f, 100.0f),
            ...
            for(std::size_t i = 0; i < positions.size(); ++i) {
                ...
                const Math::Vec2u pixel(viewport.x + viewport.width * (ndc.x + 1.0f) * 0.5f,
                    viewport.y + viewport.height * (ndc.y + 1.0f) * 0.5f);
                const auto ray =
                    make_world_ray({.view = view, .projection = projection}, pixel, resolution);
                ASSERT_TRUE(ray);
    ...
    TEST(ScenePickingTest, SelectsNearestTransformedCandidate) {
        ...
        const std::array candidates{
            ScenePickCandidate{.entity_id = 9,
                .model_matrix = Math::translate(Math::Mat4(1.0f), Math::Vec3(0.0f, 0.0f, -5.0f)),
                .local_bounds = unit_box()},
            ScenePickCandidate{
                .entity_id = 3, .model_matrix = Math::Mat4(1.0f), .local_bounds = unit_box()}};
    ...
    TEST(ScenePickingTest, PreservesWorldDistanceUnderNonUniformAndNegativeScale) {
        const Ray ray{
            .origin = Math::Vec3(0.0f, 0.0f, 10.0f), .direction = Math::Vec3(0.0f, 0.0f, -1.0f)};
        const std::array candidates{
            ScenePickCandidate{.entity_id = 1, .local_bounds = unit_box()},
            ScenePickCandidate{.entity_id = 2,
                .model_matrix = Math::scale(Math::Mat4(1.0f), Math::Vec3(-2.0f, 3.0f, 4.0f)),
                .local_bounds = unit_box()},
        };
        const auto hit = pick_scene_candidates(ray, candidates);
        ASSERT_TRUE(hit);
    ...
    TEST(ScenePickingTest, PicksLargeScaledMeshesWithoutTreatingDirectionAsZero) {
        const Ray ray{
            .origin = Math::Vec3(0.0f, 0.0f, 2.0e8f), .direction = Math::Vec3(0.0f, 0.0f, -1.0f)};
        const std::array candidates{
    ...
    TEST(ScenePickingTest, ClipsPickRayToNearAndFarPlanes) {
        const ViewProjectMatrix view_project{
            .view = Math::Mat4(1.0f), .projection = Math::perspective(60.0f, 1.0f, 1.0f, 10.0f)};
        const auto ray = make_world_ray(view_project, Math::Vec2u(0), Math::Vec2u(1));
        ASSERT_TRUE(ray);
    ...
    TEST(ScenePickingTest, RejectsInvalidViewProjectionAndResolution) {
        ViewProjectMatrix view_project{.view = Math::Mat4(1), .projection = Math::Mat4(0)};
    ...
    TEST(ScenePickingTest, IgnoresMissesInvalidIdsAndSingularModels) {
        const Ray ray{
            .origin = Math::Vec3(0.0f, 0.0f, 5.0f), .direction = Math::Vec3(0.0f, 0.0f, -1.0f)};
        const std::array candidates{
            ScenePickCandidate{.entity_id = INVALID_ENTITY_ID, .local_bounds = unit_box()},
            ScenePickCandidate{.entity_id = 7,
                .model_matrix = Math::scale(Math::Mat4(1.0f), Math::Vec3(0.0f)),
                .local_bounds = unit_box()},
            ScenePickCandidate{.entity_id = 8,
                .model_matrix = Math::translate(Math::Mat4(1.0f), Math::Vec3(5.0f, 0.0f, 0.0f)),
                .local_bounds = unit_box()}};
