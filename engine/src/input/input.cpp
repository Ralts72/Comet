...
namespace Comet {
    ...
    void Input::gamepad_sample(const size_t index, const std::optional<GamepadSample>& sample) {
        ...
        for(size_t axis = 0; axis < gamepad.axes.size(); ++axis) {
            ...
            if(!std::isfinite(value))
                value = 0;
            const bool is_stick = axis < static_cast<size_t>(GamepadAxis::LeftTrigger);
            const float minimum = is_stick ? -1.0f : 0.0f;
