...
namespace Comet {
    namespace {
        ...
        template<typename T>
        bool keep_runtime_asset_if_type_matches(
            AssetRegistry& registry,
            const AssetHandle handle,
            const AssetType indexed_type) {
            ...
            LOG_WARN(
                ...
                handle.value(),
                to_string(indexed_type));
            return false;
        }

        AssetScanReport asset_operation_error(
        ...
        bool is_safe_asset_destination(
    ...
    AssetManager::~AssetManager() {
        for(AsyncState::ScheduledAssetTask& task:
    ...
    AssetScanReport AssetManager::move_asset(
        ...
        if(!handle) {
            return asset_operation_error(
                destination, "cannot move an invalid asset handle");
        }
        if(!is_safe_asset_destination(destination)) {
            return asset_operation_error(
                destination,
        ...
        const std::filesystem::path target =
            asset_root / destination_relative;
        ...
        try {
            ...
            return asset_operation_error(
                source_relative,
                "cannot move asset with invalid metadata: "
        ...
        if(error || target_metadata_exists) {
            return asset_operation_error(
                metadata_path(destination_relative),
        ...
        const std::filesystem::path parent_relative = canonical_parent
        ...
        if(error) {
            ...
            return asset_operation_error(
                destination_relative,
                "failed to move asset source: " + error.message());
        }
        ...
        try {
            ...
            report = asset_operation_error(
                destination_relative,
                "asset database scan failed after move: "
                + std::string(exception.what()));
        }
        if(report.snapshot_updated) {
            return report;
        ...
        if(metadata_rollback_error || source_rollback_error) {
            ...
            report.issues.push_back({
                .path = destination_relative,
                .message = "asset move was rolled back because the database snapshot could not be committed"
            });
        }
        return report;
    }
    ...
    void AssetManager::process_completions() {
        ...
        for(MeshImportCandidate& candidate: completed_meshes) {
            const auto pending =
                m_async_state->pending_assets.find(candidate.handle);
        ...
        for(TextureImportCandidate& candidate: completed_textures) {
            ...
            if(!candidate.error.empty()) {
                LOG_ERROR(
                    "Failed to import modified texture asset '{}' (handle {}): {}",
                    candidate.relative_path.generic_string(),
                    candidate.handle.value(),
                    candidate.error);
                continue;
            ...
            for(const AssetHandle dependent:
                ...
                const AssetRecord* dependent_record =
                    m_database.find(dependent);
                if(dependent_record
        ...
        for(auto task = tasks.begin(); task != tasks.end();) {
            ...
            try {
                ...
                const auto pending =
                    m_async_state->pending_assets.find(task->handle);
                if(pending != m_async_state->pending_assets.end()
                   && pending->second == task->revision) {
                    m_async_state->pending_assets.erase(pending);
                }
                LOG_ERROR(
                    "Background {} task failed for asset handle {}: {}",
                    to_string(task->type),
                    task->handle.value(),
                    exception.what());
            }
    ...
    bool AssetManager::schedule_loaded_texture_refresh(
        const AssetRecord& record) {
        const AssetHandle handle = record.handle;
        const AssetRevision revision = m_database.get_revision(handle);
        const auto previous_texture = m_registry.resolve<Texture>(handle);
    ...
    bool AssetManager::schedule_refresh_task(
        const AssetHandle handle,
        const AssetRevision revision,
        ...
        try {
            ...
            const auto current_pending =
                m_async_state->pending_assets.find(handle);
            ...
            LOG_ERROR(
                "Failed to schedule {} refresh for asset handle {}: {}",
                to_string(type),
                handle.value(),
                exception.what());
            return false;
