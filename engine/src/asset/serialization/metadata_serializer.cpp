...
namespace Comet {
    namespace {
        std::runtime_error metadata_error(const std::string_view source,
            const std::string_view location, const std::string& detail) {
            return std::runtime_error("Invalid asset metadata '" + std::string(source)
        ...
        void validate_keys(const YAML::Node& root, const std::string_view source) {
            ...
            Yaml::validate_keys(root, supported, source, "<root>", metadata_error);
        }

        void validate_texture_importer_keys(
            const YAML::Node& importer, const std::string_view source) {
            const std::unordered_set<std::string> supported{"color_space", "flip_y"};
            Yaml::validate_keys(importer, supported, source, "importer", metadata_error);
        }

        YAML::Node required_child(const YAML::Node& root, const char* key,
        ...
        T read_scalar(const YAML::Node& node, const std::string_view source,
            ...
            return Yaml::read_scalar<T>(node, source, location, expected, metadata_error);
        }
    }

    std::string AssetMetadataSerializer::serialize(const AssetMetadata& metadata) const {
        if(!metadata.handle) {
        ...
        if(metadata.type == AssetType::Texture && !texture_settings) {
            throw metadata_error("<memory>", "importer",
        ...
        if(!emitter.good()) {
            throw std::runtime_error(
                "Failed to serialize asset metadata: " + emitter.GetLastError());
        }
    ...
    AssetMetadata AssetMetadataSerializer::deserialize(
        ...
        const std::uint32_t version =
            read_scalar<std::uint32_t>(required_child(root, "version", source), source,
                "version", "an unsigned integer");
        if(version != FORMAT_VERSION) {
            throw metadata_error(source, "version",
                "unsupported version " + std::to_string(version) + "; expected "
                    + std::to_string(FORMAT_VERSION));
        }

        const AssetHandle handle(
            read_scalar<AssetHandle::ValueType>(required_child(root, "guid", source),
                source, "guid", "a non-zero unsigned integer"));
        if(!handle) {
            throw metadata_error(source, "guid", "expected a non-zero value");
        }

        const std::string type_name = read_scalar<std::string>(
            required_child(root, "type", source), source, "type", "an asset type string");
        ...
        if(*type == AssetType::Texture) {
            if(!importer.IsDefined()) {
                throw metadata_error(
                    source, "<root>", "missing required field 'importer'");
            }
            ...
            import_settings = TextureImportSettings{.color_space = *color_space,
                .flip_y = read_scalar<bool>(
                    required_child(importer, "flip_y", source, "importer"), source,
                    "importer.flip_y", "a boolean")};
        } else if(importer.IsDefined()) {
            throw metadata_error(source, "importer",
                "import settings are not supported for asset type '"
                    + std::string(to_string(*type)) + "'");
        }

        return AssetMetadata{.handle = handle,
            .type = *type,
    ...
    AssetMetadata AssetMetadataSerializer::load(const std::filesystem::path& path) const {
