...
namespace Comet {
    Result<TextureData> TextureImporter::import(
        const std::filesystem::path& source_path, const TextureImportSettings& settings) const {
        int width = 0;
        int height = 0;
        const std::string source = source_path.string();
        std::unique_ptr<stbi_uc, decltype(&stbi_image_free)> pixels(
            stbi_load(source.c_str(), &width, &height, nullptr, STBI_rgb_alpha), stbi_image_free);
        if(!pixels) {
            const char* reason = stbi_failure_reason();
            return Result<TextureData>::failure("Failed to import texture '" + source + "': "
