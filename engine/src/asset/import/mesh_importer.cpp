...
namespace Comet {
    namespace {
        [[noreturn]] void fail_import(
            ...
            throw std::runtime_error(
                "Failed to import mesh '" + source_path.string() + "': "
                + std::string(message));
        }

        [[nodiscard]] const fastgltf::Accessor& get_accessor(
        ...
        void generate_normals(
            ...
            for(std::size_t index = first_index;
                index < data.indices.size(); index += 3) {
                ...
                if(i0 < first_vertex || i0 >= vertex_end
                   || i1 < first_vertex || i1 >= vertex_end
                ...
                const Math::Vec3 edge_a = data.vertices[i1].position
                    - data.vertices[i0].position;
                const Math::Vec3 edge_b = data.vertices[i2].position
                    - data.vertices[i0].position;
            ...
            for(std::size_t index = first_vertex;
                index < first_vertex + vertex_count; ++index) {
    ...
    MeshData MeshImporter::import(
        ...
        fastgltf::Parser parser;
        auto parsed = parser.loadGltf(
            source.get(), source_path.parent_path(), options,
        ...
        if(validation_error != fastgltf::Error::None) {
            fail_import(
                source_path, fastgltf::getErrorMessage(validation_error));
        }
        ...
        for(const fastgltf::Primitive& primitive: mesh.primitives) {
            ...
            if(position_attribute == primitive.attributes.end()) {
                fail_import(
                    source_path,
                    "each primitive must provide a POSITION attribute");
            }
            ...
            if(position_accessor.type != fastgltf::AccessorType::Vec3
               || position_accessor.count == 0) {
                fail_import(
                    source_path,
                    "POSITION must be a non-empty VEC3 accessor");
            }
            ...
            if(const auto* texcoord_attribute =
               ...
               texcoord_attribute != primitive.attributes.end()) {
                const fastgltf::Accessor& texcoord_accessor = get_accessor(
                    asset,
                    texcoord_attribute->accessorIndex,
                    source_path,
                    "TEXCOORD_0");
                ...
                fastgltf::iterateAccessorWithIndex<fastgltf::math::fvec2>(
                    asset,
                    texcoord_accessor,
                    [&](const fastgltf::math::fvec2 texcoord,
                        const std::size_t index) {
            ...
            if(primitive.indicesAccessor.has_value()) {
                ...
                data.indices.reserve(
                    data.indices.size() + position_accessor.count);
                for(std::size_t index = 0;
                    index < position_accessor.count; ++index) {
                    data.indices.push_back(
