...
namespace Comet {
    namespace {
        ...
        [[nodiscard]] Result<std::vector<std::filesystem::path>> collect_source_dependencies(
            ...
            return Result<std::vector<std::filesystem::path>>::success(std::move(dependencies));
        }
    }

    Result<MeshData> MeshImporter::import(const std::filesystem::path& source_path) const {
        auto source = fastgltf::GltfDataBuffer::FromPath(source_path);
        if(!source) {
