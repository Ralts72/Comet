...
namespace Comet {
    ...
    : m_device(device), m_size(size), m_buffer_type(buffer_type) {
        ...
        if(m_buffer_type == BufferMemoryType::HostVisible) {
            ...
            void* mapping = m_device->get_device().mapMemory(stage_memory, 0, vk::WholeSize);
            memcpy(mapping, data, m_size);
            m_device->get_device().unmapMemory(stage_memory);
            std::tie(m_buffer, m_memory) = create_buffer(vk::MemoryPropertyFlagBits::eDeviceLocal, usage | vk::BufferUsageFlagBits::eTransferDst);

            // copy buffer
            auto command_buffer = m_device->get_default_command_pool()->allocate_command_buffer();
            command_buffer.begin(vk::CommandBufferUsageFlagBits::eOneTimeSubmit);
            command_buffer.copy_buffer(stage_buffer, m_buffer, m_size);
            command_buffer.end();
            auto queue = m_device->get_graphics_queue(0);
            queue->submit(std::span(&command_buffer, 1), {}, {}, {});
            queue->wait_idle();

            m_device->get_device().destroyBuffer(stage_buffer);
            m_device->get_device().freeMemory(stage_memory);
        }
    }
    ...
    std::pair<vk::Buffer, vk::DeviceMemory> Buffer::create_buffer(const vk::MemoryPropertyFlags mem_props,
        ...
        auto buffer = m_device->get_device().createBuffer(buffer_create_info);

        // allocate memory
        const auto memory_required = m_device->get_device().getBufferMemoryRequirements(buffer);
        ...
        auto memory = m_device->get_device().allocateMemory(allocate_info);
        m_device->get_device().bindBufferMemory(buffer, memory, 0);
        return std::make_pair(buffer, memory);
    }

    Buffer::~Buffer() {
        m_device->get_device().destroyBuffer(m_buffer);
    ...
    void Buffer::write(const void* data) {
        ...
        void* mapping = m_device->get_device().mapMemory(m_memory, 0, vk::WholeSize);
        std::memcpy(mapping, data, m_size);
        m_device->get_device().unmapMemory(m_memory);
    }
}
