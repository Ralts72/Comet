...
namespace Comet {
    std::shared_ptr<FrameBuffer> FrameBuffer::create(
        ...
        auto attempt = try_create(
            device, render_pass, image_views, width, height);
        if(!attempt) {
    ...
    GpuResourceResult<std::shared_ptr<FrameBuffer>> FrameBuffer::try_create(
        ...
        const vk::Result result = device.get().createFramebuffer(
            &create_info, nullptr, &frame_buffer);
        if(result != vk::Result::eSuccess) {
            return GpuResourceResult<std::shared_ptr<FrameBuffer>>::failure(
                result);
        }
        ...
        LOG_TRACE("Framebuffer created, width: {} height: {}  view count: {}",
            width, height, vk_image_views.size());
        return GpuResourceResult<std::shared_ptr<FrameBuffer>>::success(
            std::move(owner));
    }
    ...
    FrameBuffer::FrameBuffer(
        ...
        : m_frame_buffer(frame_buffer),
          ...
          m_attachments(std::move(image_views)),
          m_width(width),
          m_height(height) {
    }
