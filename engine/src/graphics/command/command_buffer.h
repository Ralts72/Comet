...
namespace Comet {
    ...
    public:
        ...
        void bind_vertex_buffer(
            const VertexBufferBinding& binding, uint32_t first_binding = 0) const;

        void bind_vertex_buffers(
            std::span<const VertexBufferBinding> bindings, uint32_t first_binding = 0) const;

        void bind_index_buffer(
            const Buffer& buffer, uint64_t offset, IndexType type = IndexType::Uint32) const;
        ...
        void draw_indexed(uint32_t index_count, uint32_t instance_count = 1,
            uint32_t first_index = 0, int32_t vertex_offset = 0, uint32_t first_instance = 0) const;
    ...
    private:
        ...
        void copy_buffer(vk::Buffer src_buffer, vk::Buffer dst_buffer, size_t size,
            size_t src_offset = 0, size_t dst_offset = 0) const;

        void copy_buffer_to_image(vk::Buffer src_buffer, vk::Image dst_image,
            vk::ImageLayout dst_image_layout, const vk::Extent3D& extent,
        ...
        void transition_image_state(
            vk::Image image, const ImageState& before, const ImageState& after) const;

        void transition_buffer_state(vk::Buffer buffer, const ResourceState& before,
