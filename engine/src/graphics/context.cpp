...
namespace Comet {
    namespace {
        ...
        void log_validation_message(const uint32_t message_severity, const char* message) noexcept {
        ...
        vk::DebugUtilsMessengerCreateInfoEXT make_debug_messenger_create_info() {
            ...
            create_info.pfnUserCallback = static_cast<decltype(create_info.pfnUserCallback)>(
                vk_debug_utils_messenger_callback);
            return create_info;
        }

        vk::DebugUtilsMessengerEXT create_debug_messenger(
            const vk::Instance instance, const vk::DebugUtilsMessengerCreateInfoEXT& create_info) {
            ...
            if(result != VK_SUCCESS) {
                LOG_FATAL("Failed to create Vulkan debug messenger: {}", static_cast<int>(result));
            }
            return {messenger};
        }
    ...
    void Context::create_instance(const bool validation_requested) {
        ...
        const auto custom_extensions = get_available_names(
            requested_instance_extensions, available_extension_names, "instance extension");
        for(const char* extension : custom_extensions) {
            const bool already_enabled =
                std::ranges::any_of(enabled_extensions, [extension](const char* enabled_extension) {
        ...
        if(debug_utils_enabled) {
            m_debug_messenger = create_debug_messenger(m_instance, debug_messenger_create_info);
    ...
    void Context::pickup_physical_device(const DeviceCapabilityRequest& capability_request) {
        if(!m_instance) {
