...
namespace Comet {
    ...
    Result<std::shared_ptr<Pipeline>, GraphicsError> Pipeline::create(std::string name,
        ...
        const std::vector<vk::PipelineColorBlendAttachmentState> color_attachments(
            render_pass.get_color_attachment_count(config.subpass), config.color_blend_state);
        auto color_blend_state = create_color_blend_state(color_attachments);
