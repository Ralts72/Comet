...
namespace Comet {
    namespace {
        ...
        bool has_valid_stage_context(const ResourceUsage usage,
            ...
            constexpr StageBits SHADER_STAGES =
                ...
                | static_cast<StageBits>(PipelineStage::TessellationControlShader)
                | static_cast<StageBits>(PipelineStage::TessellationEvaluationShader)
    ...
    std::optional<ResourceState> resolve_resource_state(const ResourceUsage usage,
        ...
        switch(usage) {
            ...
            case ResourceUsage::UniformRead:
                return ResourceState{.stages = shader_stages,
            ...
            case ResourceUsage::StorageReadWrite:
                return ResourceState{.stages = shader_stages,
                    .access = Flags<Access>(Access::ShaderRead) | Access::ShaderWrite,
                    .queue_family = queue_family};
            ...
            case ResourceUsage::DepthStencilAttachmentWrite:
                return ResourceState{
                    .stages = Flags<PipelineStage>(PipelineStage::EarlyFragmentTests)
                              | PipelineStage::LateFragmentTests,
                    .access = Flags<Access>(Access::DepthStencilAttachmentRead)
    ...
    std::optional<ImageState> resolve_image_state(const ResourceUsage usage,
        ...
        const auto resource = resolve_resource_state(usage, shader_stages, queue_family);
        if(!resource) {
            return std::nullopt;
        }
