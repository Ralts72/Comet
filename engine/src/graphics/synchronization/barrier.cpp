...
namespace Comet::Graphics {
    namespace {
        bool has_valid_access_scope(const ResourceState& state) {
            return !static_cast<bool>(state.access)
                || static_cast<bool>(state.stages);
        }
        ...
        bool has_stable_queue_owner(
            ...
            const bool before_has_owner =
                before.queue_family != UNSPECIFIED_QUEUE_FAMILY;
            const bool after_has_owner =
                after.queue_family != UNSPECIFIED_QUEUE_FAMILY;
            return before_has_owner == after_has_owner
                && (!before_has_owner
                    || before.queue_family == after.queue_family);
        }
    }
    ...
    std::optional<vk::BufferMemoryBarrier2> build_buffer_memory_barrier(
        ...
        if(size == 0
           || !has_valid_access_scope(before)
           || !has_valid_access_scope(after)
