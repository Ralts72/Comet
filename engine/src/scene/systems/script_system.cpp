...
namespace Comet {
    ...
    void ScriptSystem::stop_entry(Entry& entry) noexcept {
    ...
    Result<void, Error> ScriptSystem::invoke(
        ...
        auto parameters =
            entry.script->parameters(entry.entity.get_component<ScriptComponent>().parameters);
        if(!parameters)
            return Result<void, Error>::failure(
                {key.entity.to_string() + ": " + parameters.error().message});
        entry.parameters = std::move(parameters).value();
        auto result = entry.instance->invoke(phase, entry.entity, entry.parameters, context);
        if(!result)
            return Result<void, Error>::failure(
    ...
    Result<void, Error> ScriptSystem::synchronize(Scene& scene) {
        ...
        for(const auto& [key, entity] : pending) {
            ...
            auto& entry =
                m_entries
                    .emplace(key, Entry{entity, std::move(script), std::move(instance).value(), {}})
                    .first->second;
