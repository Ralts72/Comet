...
namespace Comet {
    ...
    bool Presentation::begin_frame() {
        ...
        auto& command_buffer = m_frames.get_current_command_buffer();
        command_buffer.begin(Flags<CommandBuffer::Usage>(CommandBuffer::Usage::OneTimeSubmit));

        return true;
    }

    void Presentation::end_frame(const std::span<const QueueSemaphoreSubmit> resource_waits) {
    ...
    bool Presentation::recreate_swapchain() {
        PROFILE_SCOPE("Presentation::recreate_swapchain");
        auto& swapchain = m_context.get_swapchain();
        const auto previous = swapchain.get_active_generation()->get_config();
        ...
        const auto compatibility =
            compare_swapchain_configs(previous, swapchain.get_active_generation()->get_config());
        if(recreated)
            ...
            if(dependent->rebuild) {
                ...
                if(!result)
                    throw std::runtime_error(
                        "Cannot rebuild presentation dependent: " + result.error().message);
            }
        }
        return recreated;
    }
