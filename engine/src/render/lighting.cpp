...
namespace Comet {
    LightingData LightingData::prepare(const std::span<const RenderLight> input) {
        ...
        static_assert(offsetof(LightingData, shadow_view_projection) == MAX_LIGHTS * 64 + 16);
        static_assert(offsetof(LightingData, shadow_parameters) == MAX_LIGHTS * 64 + 80);
        ...
        for(const auto& light : input) {
            ...
            const bool color_valid = Math::is_finite(light.color) && light.color.x >= 0
                                     ...
                                     && light.color.z <= 1;
            if((!directional && !point && !spot) || !color_valid || !std::isfinite(light.intensity)
                || light.intensity < 0 || light.intensity > 10000
                || (!directional
                ...
                || (spot
                    && (!std::isfinite(light.inner_angle) || !std::isfinite(light.outer_angle)
