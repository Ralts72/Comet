...
namespace {
    ...
    public:
        void on_init() override {
            ...
            Comet::AssetScanReport initial_asset_scan =
                m_asset_manager->scan();
            log_asset_scan_issues(initial_asset_scan);
            m_asset_source_monitor =
                    std::make_unique<Comet::AssetSourceMonitor>(
        ...
        void apply_viewport_focus() {
            if(m_editor_state.mode != CometEditor::EditorMode::Edit
               || !m_viewport_panel->take_focus_request()
               || !m_selection) {
                return;
            }
            ...
            const float viewport_aspect =
                static_cast<float>(layout.image_resolution.x)
                / static_cast<float>(layout.image_resolution.y);
            static_cast<void>(CometEditor::focus_editor_camera(
        ...
        void update_viewport_texture(Comet::SceneRenderer& scene_renderer) {
            ...
            m_imgui_context->set_viewport_image(
                frame_slot,
                scene_renderer.get_viewport_color_view(frame_slot),
        ...
        void initialize_viewport_textures(
            Comet::SceneRenderer& scene_renderer) {
    ...
    private:
        void handle_asset_source_poll(
            ...
            if(result.state
               == Comet::AssetSourceMonitor::PollState::NotPolled) {
                return;
            }
            if(result.state
               != Comet::AssetSourceMonitor::PollState::Failed) {
                m_asset_source_monitor_error.clear();
                return;
            }

            const std::string error = result.issue_path.generic_string()
                + ": " + result.message;
        ...
        void acknowledge_generated_metadata(
            const Comet::AssetScanReport& report) {
        ...
        Comet::AssetScanReport refresh_project_assets(
            ...
            if(!source_state_already_observed && m_asset_source_monitor) {
                handle_asset_source_poll(
                    m_asset_source_monitor->poll_now());
            }
        ...
        void monitor_asset_sources() {
            ...
            if(result.state
               != Comet::AssetSourceMonitor::PollState::Changed) {
                return;
            }

            m_project_panel->update_scan_report(
                refresh_project_assets(true));
        }
        ...
        bool update_material(
            ...
            const bool updated = static_cast<bool>(
                m_asset_manager->update_material(handle, data));
            if(updated && !relative_path.empty()) {
                static_cast<void>(
                    m_asset_source_monitor->acknowledge(relative_path));
            }
            return updated;
        }

        bool reimport_texture(
            const Comet::AssetHandle handle,
            ...
            const std::filesystem::path relative_path =
                    record ? record->path : std::filesystem::path{};
            const bool reimported = static_cast<bool>(
