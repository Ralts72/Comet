...
| 034 | CPU/GPU 帧诊断、完成后查询、低频预算与手动 VMA 报告 | Debug/Release 各 5 个 CTest；539 单元、25 同步 GPU、10 WSI、2 契约；重复验证在旧 GLFW 初始化路径超时，留待 035 修正 |
...
阶段 3 的主线负载控制已补齐。下一项：036 可复现 Release forward 场景测量与阶段 5 边界评估，随后进入阶段 6。
已完成定期架构回顾：005、010、015、020、025、030、035；阶段边界回顾：013。下一次：040 或阶段 5 边界（取先到者）。
