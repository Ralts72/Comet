...
| 020 | 当前 Pipeline API 结构化键、弱缓存、配置生效及架构回顾 | Debug/Release 各 452 tests 通过；Release 首轮 Cocoa 超时后串行复核 |

阶段 4 本轮核心验收通过，扩展项保留在路线图；阶段 5／6 尚未完成。
阶段 3 的主线负载控制已补齐。下一项：阶段 5 Shader 编译契约，包含未开放的 specialization 值/键和反射一致性。
