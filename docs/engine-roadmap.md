...
- 容量拒绝恢复已接通：编辑器 Mesh 待办与驻留资产刷新保留身份/版本并自动重试，内容失败仍等待新事件。环境与普通纹理准备已有预估 CPU 字节预算，纹理的后台刷新在发布前保留预约；后续向 Mesh 扩展，不宣称当前已限制进程总内存。当前任务数量有界，同 Handle 只保留一个最新后继，
  ...
  这仍不是进程总内存预算；Mesh 工作集、进度、取消和索引发布预算留待后续，不把后台准备等同于全流程无卡顿。
