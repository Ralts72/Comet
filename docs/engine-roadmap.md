...
| 4 视口与交互 | 4A/4B 主链路完成，4C 进行中 | 材质创建与模板选择，再扩展内容编辑与撤销 |
| 5 渲染升级 | PBR/base-color、阴影、材质反射与内置 Shader 热发布已接通 | 环境光照／IBL、bloom、项目 Shader 资产化 |
...
1. **下一步：材质编辑闭环**（阶段 4C／5）：Project 创建材质、Inspector 选择已有模板、按属性布局编辑并指定给物体。
   先使用现有 pbr／unlit_color 与共享 metadata，不等待项目 Shader 资产化，也不为每个用户材质注册 C++ 类型。
2. **随后：环境光照**（阶段 3／5）：场景环境设置 → HDR／cubemap 资产 → 天空盒与 IBL；
   天空盒背景和照明分别验收，IBL 才解决仅有直接光照时背光面全黑的问题。
...
- 容量拒绝恢复已接通：编辑器 Mesh 待办与驻留资产刷新保留身份/版本并自动重试，内容失败仍等待新事件。大批量工作流后续按实际测量补总解码字节预算。当前任务数量有界，同 Handle 只保留一个最新后继，
...
- Project 材质资产创建与 Inspector 模板选择列为下一项，细节见阶段 5“材质编辑闭环”；
...
当前只计算直接光和阴影，未接环境光／IBL；无直接光贡献时为黑色是当前明确边界，而非靠曝光或 bloom 修补的问题。

1. **场景环境设置**：建立单一场景级环境配置，包含启用状态、环境资产引用、照明强度与旋转；背景显示可独立开关。
...
2. **环境资产与背景**：阶段 3 补 HDR 浮点解码、cubemap／mip 上传与采样、缓存及依赖，保留线性 HDR 能量。
   SkyboxPass 只绘制背景，随相机旋转而非平移，不覆盖几何；复用现有 HDR 目标、RenderGraph 和帧资源保活。
3. **PBR 环境光照**：接通漫反射 irradiance、镜面预滤波和 BRDF LUT；按金属度／粗糙度计算环境贡献，
   不能只加一项固定漫反射而让金属背光面继续无环境反射。预计算在导入／准备阶段完成，不放在每帧关键路径。
