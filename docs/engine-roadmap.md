...
下一步进入 **阶段 4B：Viewport 布局与分辨率语义分离**。先建立不依赖 ImGui/Vulkan 的布局计算，明确 panel content size、物理像素
render resolution 和保持宽高比后的 image display rect；ViewPanel 负责逻辑坐标与显示，Renderer 只接收 render resolution。随后再接入
HiDPI framebuffer scale，不把 DPI、固定分辨率预设和 RenderTarget generation 一次混做。
