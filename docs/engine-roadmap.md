...
目录快照和数据库只读候选已后台化。候选记录项目路径、数据库 generation 与输入文件的时间／大小及目录时间，主线程发布前复核；后续通知、手动 Refresh 或数据库变更使旧候选失效，未发现变化的兜底轮询不会使候选过期。`scan()` 仍供手动 Refresh 和显式资产操作同步执行，`.meta` 创建、身份／revision 更新及发布由数据库 owner 串行完成。下一轮应基于代表性项目继续测量发布阶段的逐文件复核与源签名计算，再决定是否需要缓存签名或目录结构增量索引；不直接声称全量路径已无主线程成本。
