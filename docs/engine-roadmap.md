...
仍由 500 ms 轮询兜底，macOS 根目录恢复后会尝试重建监听。已知普通资产源码及已索引导入依赖的内容变化现按路径更新快照和 revision，Shader Program 随源码更新；新增、删除、移动、`.meta`、Material／Program 结构变化和事件溢出退回完整扫描。结构性目录快照由 Worker 捕获，完成后在主线程对比并发布；已知文件的局部检查和数据库局部／全量扫描仍在主线程，后台局部复核与数据库结构增量索引尚未实现。内置 Shader 在待编译期间收到后续原生文件通知会重新计算
...
后台化资产复核需先测出局部文件状态检查或结构性全量扫描对帧时间的实际影响，再引入候选版本与过期结果协议。
局部文件检查、完整快照、数据库局部／全量扫描及扫描结果接纳已有独立 Profiler 采样；仍需在代表性项目中测量峰值与帧时间，不能仅凭平均耗时决定后台化。
合成资产基准显示结构性全量路径的成本先于已知源码局部复核成为主线程风险。
目录快照已后台化，并用 generation 丢弃显式刷新或后续通知前的旧结果；下一独立实现需拆出数据库只读扫描候选与主线程发布。现有 `scan()` 会生成 `.meta` 并更新身份／revision，
不能直接投递到 Worker。候选必须标记项目 generation 与输入快照，发布前复核过期状态；
`.meta` 创建和显式资产操作需由同一所有者串行协调。
