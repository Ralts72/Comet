...
| 5 渲染升级 | 核心 forward 链路、诊断与代表场景测量已接通 | 项目 Shader 资产化、实例化；路径追踪为远期可选扩展 |
...
6. **下一步：运行时输入快照**（阶段 6）：对照旧 `037 / 4b823cc`，接通键鼠／手柄的稳定帧状态与窗口生命周期；
   后续独立处理 Viewport 输入路由、Fixed Update 与 System／脚本，不一次堆入整个运行时。
7. **后续独立里程碑：项目自定义 Shader**（阶段 3／5）：复用材质编辑入口，接通程序资产、metadata、动态布局与发布所有权。
