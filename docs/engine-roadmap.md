...
1. **动作映射与项目输入配置**（阶段 6）：在现有 Frame／Gate 上增加具名动作与绑定，
   app／Play 共用；先打通相机与项目脚本的消费者，不引入全局 EventBus，不改变输入授权与固定步消费协议。
...
- 有界 Forward Lighting、三类 LightComponent、单方向光阴影、PBR/base-color、材质编辑、环境光照／IBL、Bloom 与有界渲染诊断已接通；架构审查与代表场景测量入口已完成。
  ...
  保留小型 forward 场景基线；延迟渲染按下节独立里程碑推进，不直接替换现有路径。
...
- 串行 System、固定更新、输入授权、Play 隔离和暂停／单步已接通；当前调用链与失败边界见
  ...
  后续物理／动画接入需回归现有暂停、单步、输入中断与失败清理契约；
...
- app/editor 文件日志已统一到当前项目 `.comet/logs/`，目录由启动入口传入，低层日志不依赖 Project；
  无路径或写入失败保留终端输出。日志轮转／保留策略、另存为、自动保存、崩溃恢复，
  以及项目设置面板、打包播放器和 CI 构建测试打包继续按需推进。
