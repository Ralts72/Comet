...
目录快照和数据库只读候选已后台化。候选记录项目路径、数据库 generation 与输入文件的时间／大小及目录时间，主线程发布前复核；后续通知、手动 Refresh 或数据库变更使旧候选失效，未发现变化的兜底轮询不会使候选过期。`scan()` 仍供手动 Refresh 和显式资产操作同步执行，`.meta` 创建、身份／revision 更新及发布由数据库 owner 串行完成。`asset_scan_benchmark` 可测真实项目临时副本，或生成简单 Lua 资产观察规模效应。本机 Release 合成样本 1,000 个资产时发布 p95 约 10 ms，5,000 个时约 54 ms；demo 仅 16 个资产，发布 p95 约 0.18 ms。合成样本不等于代表性项目，但已说明主线程发布会随规模成为风险。下一轮应在代表性项目测量逐文件复核和源签名各自的占比，再决定能否复用已验证的输入状态；不能跳过发布前复核，也不直接引入目录结构增量索引。
