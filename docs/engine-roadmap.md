...
- 容量拒绝恢复已接通：编辑器 Mesh 待办与驻留资产刷新保留身份/版本并自动重试，内容失败仍等待新事件。环境、普通纹理与 Mesh 准备已有预估 CPU 字节预算；Mesh 在描述文件不超过 64 KiB 时从输入大小与顶点／索引数估算，较大描述文件为避免主线程解析而保守预约 1 GiB。结果发布前一直占用预约；不宣称当前已限制进程总内存。当前任务数量有界，同 Handle 只保留一个最新后继，
  ...
  单批源文件及依赖合计有 512 MiB 暂存输入预算，复制前后复核大小；纹理校验复用单张 1 GiB 解码工作集估算上限。
  这仍不是进程总内存预算；Mesh 对源文件合计和输出规模设定单次 1 GiB 估算上限，Artifact 读取前也检查预算。进度、取消和索引发布预算留待后续，不把后台准备等同于全流程无卡顿。
