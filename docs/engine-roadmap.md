...
| 5 渲染升级 | 核心 forward 链路、诊断与代表场景测量已接通；项目 Shader 已可由兼容模板材质消费 | 动态材质布局、程序发布所有权、实例化、可选延迟渲染；路径追踪为远期扩展 |
...
1. **项目自定义 Shader 后续闭环**（阶段 3／5，独立里程碑）：材质通过稳定 Handle 选择已发布程序并复用固定模板布局的 GPU 路径已接通；下一项是程序 metadata、动态属性布局与 Inspector，之后收敛渲染域发布所有权。
...
1. **程序描述与不可变 CPU 版本**：随项目 Shader 资产接入，内置材质与项目程序统一描述／加载入口；
   ...
   材质可用 Handle 选择项目程序，经现有反射与固定模板契约创建 Pipeline，失败保留旧版。
   动态布局与内置材质统一描述／加载入口仍待后续验收；CPU 缓存不代表 GPU 已发布。
...
- 后续项目路径：`项目源码／程序描述 → 编译产物缓存 → 运行时加载字节码`。
  ...
  当前兼容固定模板的项目程序已复用内置 Shader 的反射、布局校验和 GPU 创建逻辑；动态布局及发布交付仍待实现。
