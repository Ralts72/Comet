...
下一步继续 **阶段 3：GPU budget snapshot 与回收策略**。`VK_EXT_memory_budget` 已按 optional capability 接入 VMA，
FrameScheduler 单调 serial 也会驱动 allocator frame index；接下来为 ResourceManager/UploadManager 提供低频 heap
budget snapshot，在批量加载和 staging pool 增长前用于诊断及保守回收，但暂不对关键 render target allocation 强制
`WITHIN_BUDGET`。
