...
| 6 游戏运行时 | 输入帧、串行 System 与暂停／单步已接通 | 游戏输入路由、脚本、dirty Transform、物理、音频 |
...
3. **运行时主线**：渲染核心和代表场景基线已验收，输入、Fixed／普通 Update 和 Play 暂停／单步已接通；下一项游戏 Viewport 输入路由，再接脚本等阶段 6 能力。
...
- Input（键鼠/手柄）、Fixed Update 与普通 Update、Native Script 生命周期和字段暴露。
  ...
  app 已用同一快照驱动相机；游戏 action mapping、Viewport 输入路由与持久回放格式仍待后续，不由底层猜测。
