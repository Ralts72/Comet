...
- Runtime 输入已单独路由；有真实需求才增加 Eject/Debug Camera 或多 Viewport。
...
- Input（键鼠/手柄）、Fixed Update 与普通 Update、Native Script 生命周期和字段暴露。
  ...
  app 已用同一快照驱动相机；游戏 action mapping 与持久回放格式仍待后续，不由底层猜测。
  Runtime 通过 Input::Gate 读取独立消费者帧；编辑器每帧只放行已聚焦、鼠标位于可见 Play 图像且无其他 UI 捕获的区域。
  离开区域释放游戏控制，重获时拦住原先按住的数字按钮，轴首帧归零；不修改平台原始输入或预建全局事件总线。
  WSI 无 UI 帧默认不放行；最小化采样中断清除旧按下／位移但保留固定更新未消费的释放。相对鼠标锁定／多玩家按需扩展。
...
- EditorMode 只含 Edit/Play；RuntimeState（Running/Paused）与之正交，支持暂停/单步，不增加 EditorMode::Paused。
  ...
  System 启动失败保留可 Stop 的副本及 Edit 原件，UI 标示 Stopped，不把运行失败误显示为 Running。
