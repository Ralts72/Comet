...
| 3 资产数据库与导入 | 主链路与任务接收背压已接通，仍有收尾 | 增量引用恢复与发布预算 |
...
下一步补全阶段 3 的完成结果／主线程发布预算，再推进阶段 4 的内容编辑验收项。
...
- 大批量刷新继续增加完成队列／主线程发布预算。当前 TaskScheduler 等待队列、AssetManager 在途及排队数量有界，
  同一 Handle 最多一个在途任务和一个最新后继；已执行的任务不强行中断。队列满会拒绝，不能等同于自动保证最终导入成功。
  下一步收敛候选结果与 future 的回收时点，让未发布结果仍占用在途槽；失败／过期结果也必须计入预算。
