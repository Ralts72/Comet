...
- 后续扩展项目设置 UI、Build Settings 和项目模板，去掉发布对源码目录的依赖。
  ...
  `Project` 要求项目始终配置启动场景，并能校验及原子写回启动场景与项目名；外部修改过的 project.json 不会被旧内存状态覆盖。编辑器 File → Startup Scene 子菜单选择场景，File 菜单也可重命名项目。输入绑定等项目设置 UI 尚未接入；编辑器自身快捷键已有独立的用户偏好设置，不属于项目输入配置。更完整的项目模板也仍待实现。
