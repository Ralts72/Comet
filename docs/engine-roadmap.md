...
仍由 500 ms 轮询兜底，macOS 根目录恢复后会尝试重建监听。已知普通资产源码及已索引导入依赖的内容变化现按路径更新快照和 revision，Shader Program 随源码更新；新增、删除、移动、`.meta`、Material／Program 结构变化和事件溢出退回完整扫描。结构性目录快照和数据库只读候选由 Worker 捕获／准备，主线程复核输入、写入缺失 `.meta` 并发布索引；已知文件的局部检查、数据库发布阶段输入复核与源签名计算仍在主线程，后台局部复核与数据库结构增量索引尚未实现。内置 Shader 在待编译期间收到后续原生文件通知会重新计算
...
`editor.file_watch.quiet_period_ms`（0..2000，默认 200），重启编辑器后生效。Windows 原生后端、后台局部复核、
...
合成资产基准显示结构性全量路径的成本先于已知源码局部复核成为主线程风险。
