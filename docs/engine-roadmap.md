...
- 容量拒绝恢复已接通：编辑器 Mesh 待办与驻留资产刷新保留身份/版本并自动重试，内容失败仍等待新事件。环境、普通纹理与 Mesh 准备已有预估 CPU 字节预算；Mesh 在描述文件不超过配置阈值（默认 64 KiB）时从输入大小与顶点／索引数估算，较大描述文件为避免主线程解析而保守预约单次额度（默认 1 GiB）。结果发布前一直占用预约；不宣称当前已限制进程总内存。当前任务数量有界，同 Handle 只保留一个最新后继，
  完成候选在发布／丢弃前仍占在途槽，失败／过期也计入消费预算；2 ms 是非抢占软预算，不保证整帧耗时。
  已执行的任务不强行中断。队列满会拒绝，不能等同于自动保证最终导入成功。
  自动扫描只重查新增／变化的 Mesh 与尚未成功建立 Artifact 的 Mesh；后者覆盖缺失 `.bin` 时无法索引依赖的恢复路径。手动 Refresh 仍检查所有 Mesh，允许修复被外部删除的缓存，不引入每帧产物扫描。
  外部文件复制／校验已改为后台准备，主线程复核目标并发布；队列按请求数有界，
  单批源文件及依赖合计的暂存输入预算默认 512 MiB，复制前后复核大小；纹理和 Mesh 的单次工作集额度默认各 1 GiB。以上导入额度与后台队列容量由 `config/common.yaml` 的 `assets` 配置提供给 app／Editor；格式正确性约束仍固定在解析器内。
  这仍不是进程总内存预算；Mesh Artifact 读取前也检查预算。进度、取消和索引发布预算留待后续，不把后台准备等同于全流程无卡顿。
...
首轮已让资产与内置 Shader 共用目录变化提示：macOS 使用 FSEvents，回调收集有界路径提示；其他平台或失效根目录
仍由 500 ms 轮询兜底，macOS 根目录恢复后会尝试重建监听。已知普通资产源码及已索引导入依赖的内容变化现按路径更新快照和 revision，Shader Program 随源码更新；新增、删除、移动、`.meta`、Material／Program 结构变化和事件溢出退回完整扫描。结构性目录快照、已知文件的局部状态复核和数据库只读候选由 Worker 捕获／准备，主线程比较局部结果、复核数据库输入、写入缺失 `.meta` 并发布索引；数据库发布阶段输入复核与源签名计算仍在主线程，数据库结构增量索引尚未实现。局部复核期间的新通知及手动 Refresh 使旧结果失效，任务队列暂满时保留待查路径。内置 Shader 在待编译期间收到后续原生文件通知会重新计算
200 ms 截止时间，兜底复核本身不作为新事件。项目 Shader 的扫描事件现按 Program Handle 延后
200 ms 排队编译；每次相关 revision 变化更新该程序的截止时间，显式 Refresh 不等待。扫描和 revision
更新仍立即进行，其他资产导入不参与这次防抖。内置与项目 Shader 共用 `config/editor.yaml` 的
`editor.file_watch.quiet_period_ms`（0..2000，默认 200），重启编辑器后生效。Windows 原生后端、
其他资产的按键防抖与溢出恢复仍待实现，不等同于下述完整目标。内置 Shader 的一次候选仍整组提交，
但 Worker 只重新编译输入有变化或上次失败的阶段；其余成功阶段复用已验证的字节码，发布前再次核对整组输入。
局部文件检查、完整快照、数据库局部扫描／全量准备／发布及扫描结果接纳已有独立 Profiler 采样；编辑器更新、资产更新与引用恢复也有采样入口，可与 `Engine::Frame` 和渲染统计并看。各标签的最大值未必来自同一帧；仍需在代表性项目中测量峰值与帧时间，不能仅凭平均耗时判断后续优化优先级。
合成资产基准显示结构性全量路径的成本先于已知源码局部复核成为主线程风险；局部文件状态检查现已移到后台。
目录快照和数据库只读候选已后台化。候选记录项目路径、数据库 generation 与输入文件的时间／大小及目录时间，主线程发布前复核；后续通知、手动 Refresh 或数据库变更使旧候选失效，未发现变化的兜底轮询不会使候选过期。发布前复核通过后，源签名复用候选记录的文件状态，避免再次读取已验证文件；新生成的 `.meta` 等候选未记录的文件仍现场读取。`scan()` 仍供手动 Refresh 和显式资产操作同步执行，`.meta` 创建、身份／revision 更新及发布由数据库 owner 串行完成。`asset_scan_benchmark` 可测真实项目临时副本，或生成简单 Lua 资产观察规模效应；`--profile` 在 RelWithDebInfo 构建下输出准备、发布前复核、源签名与整个发布阶段的 Profiler 采样。本机 Release 合成样本 1,000／5,000 个资产的发布 p95 从约 10／54 ms 降至约 6.6／34 ms；demo 仅 16 个资产，不能代表大项目。合成样本仍不等于真实项目，且发布前逐文件复核仍在主线程；下轮应在代表性项目测量峰值与编辑器帧时间，再判断是否值得进一步调整发布协议，不能为了提速直接跳过过期校验。
...
- 已提前补齐最小项目入口：`project.json` 保存版本、名称、可选启动场景与输入绑定；app／editor 接受项目目录／描述文件路径。
