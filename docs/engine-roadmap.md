...
| 6 游戏运行时 | 输入、固定更新、System、Lua 与 Transform 显式写入／增量同步已接通 | 物理、音频、脚本 API 与模块依赖 |
...
1. **物理最小闭环**（阶段 6）：先按角色 demo 的需求评估 Jolt／Bullet，再接刚体／碰撞体组件与固定步模拟；
   明确物理与 Transform 的写入权、层级限制及暂停／单步／Stop 清理，不直接引入完整角色框架。
2. **项目自定义 Shader**（阶段 3／5，独立里程碑）：沿用材质编辑入口，接通程序资产、metadata、动态布局与发布所有权。
   不作为阶段 6 脚本的前置；有真实内容需求时独立推进。

阶段 3 导入扩展与阶段 4 内容编辑待办继续保留。物理、原生文件监听、材质手势保存／资产撤销
...
- Scene 的 ID／UUID／父子索引与 Transform 显式写入已接通：属性编辑、Undo、加载、Lua 和相机控制
  ...
  后续物理／动画明确写入时点与所有权；持续用直接 TRS 组合校验缓存，按实际规模衡量脏子树标记成本。
