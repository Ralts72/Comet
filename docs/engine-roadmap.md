...
下一步推进阶段 5 的后台 Shader 编译、发布和热更新；阶段 3 的导入扩展及阶段 4 的内容编辑待办继续保留。
...
1. 编辑器后台编译与发布：请求 revision、输入快照复核、失败保留旧版本、在途帧寿命。
2. 项目 Shader／程序资产与布局生成：补复杂参数、顶点输入／stage 间接口、外部字节码校验。
