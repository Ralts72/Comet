...
| 5 渲染升级 | 核心 forward 链路、诊断与代表场景测量已接通；项目 Shader 的基础动态材质属性与单渲染域发布版本已接通 | 多消费者发布、复杂 Shader 接口、实例化、可选延迟渲染；全局光照按内容需求扩展，路径追踪为远期可选模式 |
| 6 游戏运行时 | 输入、固定更新、System、Lua、Transform、基础刚体／碰撞及短音效播放已接通 | Gameplay 纵向闭环、脚本 API 与模块依赖；动画、骨骼、音频和高级模拟按需求扩展 |
...
- glTF 多 mesh 子资产、node transform、material、animation、skin、morph target 需明确导入契约后逐项扩展；
  动画与骨骼的消费、版本和失败边界见阶段 6，不把导入字段支持误认为播放和蒙皮已完成。
...
使旧候选失效。内置与项目 Shader 按受影响程序尾沿防抖，当前使用 200 ms 编辑器内部默认值；
...
- **尾沿防抖**：以受影响资产／Shader 程序批次为合并键，而不是全局定时器；共享 include 可使多个程序分别变脏。
  ...
  当前 quiet_period 为 200 ms，按真实输入和编译耗时测量后调整；不把它作为项目或用户设置，
  也不将此值视为行业标准。
...
当前 IBL 只提供全局环境入射光，不计算场景几何遮挡后的间接反弹；方向光阴影也不是 GI。
先用真实的室内／室外参考场景确定静态与动态物体比例、目标设备及画质预算，再独立选择烘焙光照贴图／探针
或实时 GI 路径，不同时承诺多套算法，也不把屏幕空间近似称为完整场景 GI。

1. **静态内容优先验收**：若场景主要静态，先明确第二套 UV／探针布置、烘焙输入与产物资产身份，
   保存几何、材质、光源和环境的依赖；输入变化使旧产物失效，不在普通运行帧中烘焙。
   渲染消费间接漫反射时保留现有直接光、阴影和 IBL 边界，避免重复计入环境能量。
2. **动态对象与实时扩展**：为移动物体定义采样或回退策略；只有静态方案不足且目标设备满足预算时，
   再评估实时探针、屏幕空间或光追方案的更新、遮挡、时间累积和失效规则。
   路径追踪作为独立渲染模式可提供参考图，不默认成为实时光栅 GI 的依赖。
3. **验收**：对比无 GI、仅 IBL 和间接光的线性 HDR 结果；覆盖漏光、几何／灯光修改后的失效、
   缺失产物回退、场景切换、显存与 GPU 耗时。GI 不阻塞阶段 6 Gameplay 闭环。
...
- 基础 Jolt 刚体／碰撞已接通：Play／app 共享 System，Scene 保存配置，Stop 销毁模拟状态；
  ...
  Play／app 可按组件配置启动自动播放；Lua 也可通过本实体的 Audio Source 请求一次短音效，源实体同帧销毁不截断播放。
  Stop 销毁设备与实例；无输出设备时静音继续运行。
  暂停中的声音冻结、流式音乐、空间音效与混音编辑器尚未实现，按实际内容需求独立验收。
  AI 与 Runtime UI 引入依赖前仍按实际 demo 需要评估。
- **动画与骨骼（阶段 3 导入 + 阶段 6 运行时 + 阶段 5 蒙皮渲染，未实现）**：
  先以 glTF 节点 TRS 动画和骨骼角色各一个样例验收层级、joint／inverse bind matrix、动画 clip／通道的导入，
  将可复用定义与 Scene 实例的播放时间、速度、循环状态分开；缺失节点、无效权重与不兼容重导入须诊断，
  不用源文件索引充当跨导入的稳定资产身份。
  Animator System 采样局部姿态并合成骨骼矩阵，遵守暂停／单步、Play/Edit 隔离和场景替换协议；
  同一 Mesh 的主绘制与阴影使用一致姿态，在途帧保留所需骨骼缓冲，不把逐帧姿态写回资产或场景文件。
  首版完成单 clip 播放、循环和确定性时间推进，再独立接混合、状态机、root motion、morph target 与重定向；
  root motion 写实体 Transform 时必须与物理 body 的所有权和更新时点协调，不让动画与动态刚体同时覆盖同一变换。
  验收保存重开、app／Play 一致、暂停／单步、主绘制／阴影一致、重导入与资源缺失回退。
- **高级模拟（按内容需求独立扩展，未实现）**：现有固定步 Jolt 刚体和规划中的粒子 CPU 模拟是基础，
  不代表布料、软体或流体已经接通。先用具体内容样例决定是否引入其中一种，再明确模拟数据归运行实例、
  配置归场景／资产、固定步更新与渲染快照分离；碰撞交互、容量与时间预算、暂停／单步、
  Stop 后清理和失败回退须与现有 Runtime 协议一致。CPU／GPU 实现按目标设备和规模测量选择，
  不先创建通用 SimulationManager，也不把不可重复的 GPU 结果宣称为确定性回放。
...
现有输入、System、Lua、物理和音频只是运行时底座，尚未证明项目能完整实现一段玩法。
引擎提供稳定的实体访问、生命周期和跨系统通知边界；规则、目标、数值及表现由项目的场景、组件和脚本定义，
不把具体玩法固化为引擎内置 `GameplayManager` 或每种玩法一个引擎 System。

1. **受控脚本 API**：Lua 能通过显式的实体引用读取必要组件、修改本场景实例的 Transform／玩法状态，
   并请求创建、销毁实体；结构变更在明确的更新边界提交，不在 `Scene::each` 遍历或物理回调中即时改结构。
   持久引用使用 EntityUuid，运行时引用需校验场景世代和实体存活；不向脚本暴露 EnTT、GPU 资源或 Editor 对象。
   脚本模块依赖、失败隔离与热重载沿用阶段 6 的 Lua 契约，不另建平行脚本生命周期。
   当前已接入按 UUID 查找及自身实体引用，支持本地 Transform 读写，并校验场景世代与实例身份；
   创建／销毁请求在启动、固定更新、普通更新各阶段的所有 System 执行后提交，失败／Stop 丢弃未提交请求。
   新实体暂只有默认组件；编辑器实体引用选择器和运行中组合组件的受控接口仍待实现。
2. **Gameplay 通知与状态**：先为实际消费者接通碰撞／触发通知，标明固定步产生、交付顺序和销毁后的失效规则；
   只把相关实体的通知交给其脚本或 System，不预建全局 EventBus。分数、进度等会话状态归运行中的场景实例，
   与可序列化的初始配置分开；暂停、单步、Stop 与重新 Play 必须有明确的保留／重置行为。
   当前已有固定步接触进入／离开、无碰撞响应的 Trigger 及相关 Lua 回调；普通更新末交付，失效实体跳过，
   Stop／失败清空暂存通知。静态 Trigger 的休眠接触结束尚不等于几何离开；若样例需要持续占用语义，
   应在玩法闭环前补上明确的占用检测。运行场景已有受限的共享会话值，Stop／失败／再次启动清空，
   Play 不写回 Edit 且不序列化；它只承载少量跨脚本状态，不替代实体组件或持久存档。
3. **可玩样例验收**：用一个小型项目串起输入驱动、可交互目标、触发判定、计分或目标完成、声音与可见反馈，
   并支持重新开始。先用场景内表现验证反馈；独立 HUD、角色控制器与跨场景关卡切换按样例需要分别接入，
   不把它们作为首个玩法闭环的前置条件。App 与 Editor Play 运行同一份项目规则，Edit 不执行或保存运行态。
   demo 已将方向键移动、目标 Trigger、会话分数、提示音、目标消失与立方体上升串联；Stop／再次 Play 从 Edit 场景重启。
   CPU 测试覆盖触发、音频请求、场景副本与重启；完整闭环仍需在 app 和 Editor Play 手工验收。

验收：从项目文件重新打开后能完成整段玩法；输入、触发、暂停／单步、重新开始及 Stop 后再次 Play 行为一致；
缺失实体或脚本错误有诊断且不污染 Edit 场景；核心规则有不依赖渲染设备的测试，至少一次 app／Play 手工走通。
...
1. **区分两种配置 UI**：编辑器编辑项目默认动作、绑定、反向／死区及显式禁用，校验成功后原子保存项目设置；
   ...
   当前编辑器已可在 Edit 模式修改项目默认动作、绑定、倍率和死区，并原子保存；键盘录入及跨动作共用提示已接入。
   设置后需重新进入 Play／重启 app。玩家覆盖、上下文级冲突判断与游戏内改键 UI 仍待后续独立验收。
...
- 编辑器内已有 File → Open Project 路径入口，与 Open Scene 分开；可输入目录或 project.json。
  ...
  再释放旧场景／选择／历史／资产缓存与监视器；ImGui 布局作为用户偏好跨项目共享，禁止旧项目结果发布到新项目。
  验收：无需命令行即可选项目；取消／失败不丢修改；同 Handle 的两个项目不串用资源或缓存。
- 后续扩展项目设置 UI、Build Settings 和项目模板，去掉发布对源码目录的依赖。
  编辑器已在项目本地状态中记录上次场景；app 不消费该状态。
  编辑器已支持从新目录创建最小项目，生成 project.json 与含主相机的启动场景；项目设置修改并校验成功后自动原子保存，不单独增加 Save Project 按钮。
  `Project` 要求项目始终配置启动场景，并能校验及原子写回启动场景与项目名；外部修改过的 project.json 不会被旧内存状态覆盖。编辑器 Project 菜单可重命名项目、选择启动场景，并从 Settings → Input 面板修改默认输入动作；编辑器自身快捷键已有独立的用户偏好设置，不属于项目输入配置。更完整的项目模板也仍待实现。
  输入设置页编辑项目默认绑定；游戏内改键保存玩家覆盖，具体边界与验收见阶段 6“输入配置与交互扩展”。
  Save Scene 仅保存场景，不连带重写项目描述；只有启动场景等项目设置变化才保存项目。项目缓存与场景会话放 `.comet/`，跨项目的最近列表和 ImGui 布局放用户目录。
