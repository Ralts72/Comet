...
仍由 500 ms 轮询兜底，macOS 根目录恢复后会尝试重建监听。已知普通资产源码及已索引导入依赖的内容变化现按路径更新快照和 revision，Shader Program 随源码更新；新增、删除、移动、`.meta`、Material／Program 结构变化和事件溢出退回完整扫描。两条路径仍在主线程执行，后台局部复核与数据库结构增量索引尚未实现。内置 Shader 在待编译期间收到后续原生文件通知会重新计算
