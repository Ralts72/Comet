...
1. **项目自定义 Shader**（阶段 3／5，独立里程碑）：沿用材质编辑入口，接通程序资产、metadata、动态布局与发布所有权。
...
1. **程序描述与不可变 CPU 版本**：随项目 Shader 资产接入，内置材质与项目程序统一描述／加载入口；
   ...
   项目侧 `.shader` 身份、阶段引用、后台 CPU 编译、输入快照和损坏缓存重建已接通；内置材质统一描述／加载入口、
   反射接口与项目程序消费仍待后续同一验收项完成，现有 CPU 缓存不代表 GPU 已发布。
