...
- 编辑器源文件的移动、删除、导入和创建已由 `EditorAssets` 经 `AssetSourceOperations` 操作其持有的
  `AssetDatabase`；`AssetManager` 借用同一索引并处理导入、加载与发布。下一条真实资产纵切先审查
  新消费者是否仍需同时看到源编辑与运行时加载 API，并通过调用方迁移和失败测试收窄入口。
