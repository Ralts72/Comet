...
| 3 资产数据库与导入 | 主链路及 Mesh 导入 UI 可用，仍有收尾 | 任务背压、完成预算、更多导入能力 |
...
1. **完善内容编辑入口**：实体／组件增删、名称、层级和 duplicate 已支持撤销，模型导入与资产拖拽已接通，接下来补齐重新打开场景的资产加载闭环。
   保持修改、world transform 更新、提取与绘制的时序一致；结构修改不能简单套属性快照。
2. **按需通知事件**：编辑命令入口稳定后，再接真实一对多通知；不预建全局 EventBus，详见阶段 4。

另有两项应独立安排：阶段 3 的任务背压／发布预算，以及 WSI 失败后的无呈现重试。
前者是负载控制，后者是可恢复性缺口，不用一个大重构捆绑完成。
...
- 搜索、复制粘贴、Prefab MVP；重新打开场景时加载已保存的资产引用，补齐重启后的可见性闭环。
