...
| 3 资产数据库与导入 | 主链路及有界导入队列可用，仍有收尾 | 完成预算、更多导入能力 |
...
3. **渲染主线**：后台任务背压已接通，下一项补完成预算，再按阶段 5 推进多布局材质、PipelineKey、Shader 更新及多 pass。

另有两项应独立安排：阶段 3 的发布预算，以及 WSI 失败后的无呈现重试。
...
- TaskScheduler 等待队列有界；AssetManager 限制在途数和待执行数，同 Handle 的最新待执行 revision 替换旧请求。
  ...
  完成结果仍一次处理当前批次，下一项增加完成队列／主线程发布预算，不将任务数上限冒充字节或时间上限。
