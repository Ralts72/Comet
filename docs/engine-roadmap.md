...
2. **进行中：环境光照**（阶段 3／5）：场景环境设置、HDR／cubemap 资产与天空盒背景已接通；
   下一步接环境产物缓存／准备及 IBL。背景和照明分别验收，IBL 才解决仅有直接光照时背光面全黑的问题。
...
当前已接天空盒背景，材质仍只计算直接光和阴影，未接环境光／IBL；无直接光贡献时为黑色是当前明确边界，而非靠曝光或 bloom 修补的问题。

1. **场景环境设置（背景已接通，照明待扩展）**：单一场景级配置已保存环境引用、背景开关、背景强度与 Y 旋转。
   IBL 时增加独立照明开关／强度，不复用背景强度控制照明。
...
2. **环境资产与背景（基础链路已接通）**：独立 Environment 类型接 2:1 Radiance HDR、RGBA16F cubemap／背景 mip 上传与采样。
   ...
   场景引用恢复、驻留重载与失败保留复用资产链路；首次加载和外部文件复制／校验仍同步。
   下一轮与 IBL 一起补环境 Artifact、输入快照／算法版本缓存键、后台首次准备、字节预算及缓存原子发布；
   不把当前有界尺寸和后台重载描述成异步首次加载。EXR、六面图片及更高背景分辨率按实际内容需求扩展。
