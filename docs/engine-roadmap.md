...
| 6 游戏运行时 | 输入、固定更新、System、Lua、Transform 显式写入及基础刚体／碰撞已接通 | 音频、脚本 API 与模块依赖；物理高级能力按需求扩展 |
...
1. **音频最小闭环**（阶段 6）：根据 demo 的实际声音需求评估 miniaudio，明确声音资源、播放实例和 Play／Stop 生命周期，
   再接实体上的发声组件与固定／普通更新边界；不先做完整混音编辑器。
...
- 基础 Jolt 刚体／碰撞已接通：Play／app 共享 System，Scene 保存配置，Stop 销毁模拟状态；
  ...
  音频（候选 miniaudio）、动画/AI 与 Runtime UI 引入依赖前仍按实际 demo 需要评估。
