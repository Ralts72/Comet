...
更新：2026-09-23。目标是能完成小型 3D 项目的编辑器型引擎，先打通数据和编辑闭环，再扩展渲染与运行时能力。
...
1. **New Script 创建闭环**（阶段 4／6，可独立推进）：Project 创建最小模块式 Lua 文件，沿用现有资产创建约束；不为模板增加运行时阶段或宿主 API。
2. **内容模型后续验证**（阶段 3／5／7，按实际需求）：同一 Program 的两个材质实例、独立参数值和失败版本回退已有定向 UI／GPU 验证；只有出现不同的材质编辑契约或效果级参数消费者时，才决定 `.shader` metadata 是否迁移，不新增空壳材质资产类型。
3. **项目 Shader 后续闭环**（阶段 3／5）：稳定 Handle、基础动态属性和单渲染域已发布版本已接通；多消费者发布、发布包交付与复杂接口各按真实消费者独立验收，不作为脚本创建的前置。

阶段 3 导入扩展与阶段 4 内容编辑待办继续保留。物理高级能力、原生文件监听、材质手势保存／资产撤销，
...
- **New Script（独立验收项）**：Project 在目标目录创建 `.lua` 与稳定身份；默认模板采用模块表、具名 `update` 方法和文件末尾 `return`，
  其他已有生命周期方法及 `properties` 按需添加，不生成所有空的占位函数。复用资产创建的路径／重名校验、失败清理、扫描和选中流程；
  重开后可分配给实体并在 app／Play 执行。模板不生成 `group.new()`、`setmetatable`、`self.parent` 或尚未支持的阶段；
  自定义 `self:method()` 须等实例方法查找协议明确后再开放。
