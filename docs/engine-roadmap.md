...
| 6 游戏运行时 | 输入与项目动作映射、固定更新、串行 System、暂停／单步与 Lua Script 已接通 | Transform 写入契约、物理、音频、脚本 API 与模块依赖 |
...
1. **Transform 写入契约与快照读取**（阶段 6）：先收敛脚本、Inspector／Gizmo、Undo 与加载的修改入口，
   再按实际写入推进脏标记；保留即时世界矩阵查询，不只是把原算法搬进 TransformSystem。详见阶段 6 验收。
...
以下按实际游戏内容需求分别验收，不改变下一步优先推进 Transform 写入契约的顺序。
