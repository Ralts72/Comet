...
| 5 渲染升级 | 材质分层、Inspector、反射、Pipeline 变体与 CPU 编译入口已接通 | 热更新、多 pass、线程边界 |
...
材质布局重建、呈现／场景边界和完整目标事务已收敛；辅助线 Shader 热更新按实际需求暂缓，下一步参考旧 026 `6d9f365` 接通驱动 PipelineCache 持久化；
