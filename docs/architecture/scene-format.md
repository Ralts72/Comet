...
`SceneSerializer` 的 serialize／deserialize／clone／save／load 均返回公共 `Result`。
JSON 校验只在序列化边界捕获 `Json::Error`，组件回调抛出的其他异常不会伪装成普通格式错误。
保存先完成编码，再原子写入；自动创建缺失的父目录，I/O 失败返回包含目标路径的错误。
编辑器只有加载成功才激活候选 Scene，只有保存成功才更新文档路径；Play 克隆失败保持原 Edit Scene，允许修正后重试。
